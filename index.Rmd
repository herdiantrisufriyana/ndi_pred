...
```{r Severe ROP prediction - Write baseline 2, eval=FALSE, include=FALSE}
stack_prediction(
...
```{r Severe ROP prediction - Write stack 1, eval=FALSE, include=FALSE}
stack_prediction(
...
```{r BPD prediction - Write baseline 2, eval=FALSE, include=FALSE}
stack_prediction(
...
```{r BPD prediction - Write stack 1, eval=FALSE, include=FALSE}
stack_prediction(
...
```{r Mod/sev BPD prediction - Write baseline 3, eval=FALSE, include=FALSE}
stack_prediction(
...
```{r Mod/sev BPD prediction - Write stack 1, eval=FALSE, include=FALSE}
stack_prediction(
    modeval_data_set$bpd_moderate_severe$baseline |>
...
```{r Mod/sev BPD prediction - Write stack 2, eval=FALSE, include=FALSE}
stack_prediction(
    modeval_data_set$bpd$residual
    , data_dir = "inst/extdata/modeval_data_set/"
    , model_dir = "inst/extdata/model/"
...
eval <- list(calib = list(), dec = list(), disc = list)
eval_plot <- list(calib = list(), dec = list(), disc = list)
```
...
```{r Arrange evaluation plots, include=FALSE}
eval_plot$calib <-
  eval$calib |>
  lapply(lapply, arrange_calib_plot)
```

```{r echo=FALSE, fig.height=6, fig.width=5}
eval_plot$calib[str_detect(names(eval_plot$calib), "^hearing")]
```
