        ...
        names(.x[[x]]) |>
          lapply(
            ...
            .x[[x]][[y]] |>
              ...
              summarize(n = n(), .groups = "drop") |>
              mutate(pred = sum(!.x[[x]][[y]] %in% c("id", "outcome")))
          ) |>
          reduce(rbind)
      ) |>
      reduce(rbind)
...
dim_red_train_regression |>
  mutate(outcome = paste0(outcome, ifelse(pred > max_pred, "*", ""))) |>
  select(-set) |>
  kable(
...
```{r Conduct principal component analysis}
dr_model <-
  dr_input |>
...
ggarrange(
  ggarrange(
    ggarrange(
      dr_model_weight_plot$hearing$baseline +
      ...
      , nrow = 1
      , widths =
        c(nrow(dr_model_weight_plot$hearing$baseline$data) + 2
          , nrow(dr_model_weight_plot$rop_severe$baseline$data)
        )
      , labels = LETTERS[1:2]
    )
    , NULL
...
Figure 5. Dimensional reduction model weights for predictors of: (A) failed hearing test baseline prediction; (B) Severe ROP baseline prediction; and (C) survival baseline prediction. PVE, proportion of variance explained
...
```{r Survival prediction - Write baseline, eval=FALSE, include=FALSE}
...
```{r Survival prediction - Write residual, eval=FALSE, include=FALSE}
compute_residual(
    base_df_list =  modeval_data_set$survive$baseline
    , resid_df_list = modeval_data_set$survive$residual
  ...
  imap(
    ~ .x |>
      write_csv(
        paste0(
          "inst/extdata/modeval_data_set/survive_residual1_", .y, ".csv"
        )
      )
  )
```

```{r Survival prediction - Baseline results, include=FALSE}
survive_baseline1 <-
  obtain_outcome_pred(
...
```{r Survival prediction - Residual results, include=FALSE}
survive_residual1 <-
  obtain_outcome_pred(
...
survive_stack <- stack_prediction(survive_baseline1, survive_residual1)
```
