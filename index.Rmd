...
univar_reg <-
  ...
  lapply(
    \(x)
    univar_reg |>
      setdiff(ifelse(x == "survive", NA, "death_age")) |>
      `names<-`(
        univar_reg |>
          setdiff(ifelse(x == "survive", NA, "death_age"))
      ) |>
      ...
      lapply(
        \(x)
        suppressWarnings(glm(x, family = binomial(), data = processed_data))
      ) |>
      lapply(tidy) |>
...
sig_var <-
  ...
  right_join(
    expand.grid(
      V1 = colnames(processed_data)
      , V2 = colnames(processed_data)
      , stringsAsFactors = FALSE
    )
    , by = join_by(V1, V2)
  ) |>
...
```{r}
sig_var[var$dependent, , drop = FALSE] |>
  as.data.frame() |>
  rownames_to_column(var = "outcome") |>
  gather(variable, predictor, -outcome) |>
  filter(predictor == 1) |>
  select(-predictor) |>
  arrange(factor(outcome, unique(outcome)))
```
