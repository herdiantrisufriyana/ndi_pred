...
modeval_data_set <-
  ...
  imap(
    ~ list(
        ...
        , residual =
          modeval_data |>
          select_at(c(var$s, var$t, .y, .x$residual)) |>
          unique()
      ) |>
      imap(
...
```{r Write datasets for modeling & evaluation, eval=FALSE, include=FALSE}
save_dir <- "inst/extdata/modeval_data_set/"
if(!dir.exists(save_dir)) dir.create(save_dir, recursive = TRUE)

seq(max(sapply(pred_seq, length))) |>
  lapply(
    \(x)
    modeval_data_set[setdiff(unique(sapply(pred_seq, \(y) y[x])), NA)] |>
      imap(
        ~ names(.x) |>
          lapply(
            \(y)
            names(.x[[y]]) |>
              lapply(
                ...
                .x[[y]][[z]] |>
                  write_csv(
                    paste0(
                      "inst/extdata/modeval_data_set/"
                      , "stack", x, "_", .y, "_", y, "_", z, ".csv"
                    )
                  )
              )
          )
