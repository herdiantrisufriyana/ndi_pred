...
multivar_reg  |>
  ...
  select_at(c("outcome", "variable", "term", "OR", "LB", "UB", "p.value")) |>
  mutate_at(4:7, round, 3) |>
  mutate_at(c("outcome", "variable"), as.character) |>
  left_join(
    multivar_adjustment |>
...
es_plots <-
  ...
  lapply(
    ...
    univar_reg  |>
      ...
      mutate(seq = seq(n())) |>
      gather(metric, value, -seq, -outcome, -variable_term) |>
      ...
      geom_errorbar(
        aes(ymin = LB, ymax = UB)
        , alpha = 0.75, width = 0.2, na.rm = TRUE
      ) +
...
```{r figure-4, echo=FALSE, fig.height=21, fig.width=7}
ggarrange(
  ...
  , widths = 7
  , heights =
    c(nrow(es_plots$survive$data) + 2
      , sapply(es_plots, \(x) nrow(x$data))[-1]
    )
)
```
