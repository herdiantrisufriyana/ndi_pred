...
```{r Conduct correlation tests per pair with PS, include=FALSE}
...
correlation_matrix_ps <-
  ...
  pblapply(
    \(x)
    readRDS(paste0("data/correlation_matrix_ps/",x$V1,"_",x$V2,".rds"))
  ) |>
  lapply(
    \(x)
...
saveRDS("data/correlation_matrix_ps.rds")
```
...
correlation_matrix_non_ps_failed[
  sapply(
    correlation_matrix_non_ps_failed
    , \(x) str_detect(x$obj, "non-missing")
  )
]

correlation_pair |>
  filter(V1 == "death_age" & V2 == "hearing")

ms_added_data0[, c("death_age", "hearing", "id")] |>
  unique() |>
  filter(!is.na(hearing)) |>
  filter(!is.na(death_age))
  group_by(hearing) |>
  summarize(n = n(), .groups = "drop")
```
