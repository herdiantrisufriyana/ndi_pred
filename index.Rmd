---
title: "Early prediction of neonatal outcomes among very preterm infants using multivariable trajectories: Development and validation"
author: "Yung-Chieh Lin, Herdiantri Sufriyana, Chien-Jung Ho, Ting-Chun Yeh, Chao-Ching Huang, Emily Chia-Yu Su"
date: "2024-09-10"
...
```{r Compile multivariate regression results, include=FALSE}
...
```{r Survival prediction - Baseline 1 evaluation, eval=FALSE, include=FALSE}
survive_baseline1_eval_df_list <-
  list(
    survive_baseline1_rr = survive_baseline1_rr
  )

survive_baseline1_eval_discrimination <-
  survive_baseline1_eval_df_list |>
  lapply(\(x) lapply(lapply(x, select, -id), discrimination, seed = seed))

survive_baseline1_eval_discrimination |>
  saveRDS("data/survive_baseline1_eval_discrimination.rds")
```

```{r Survival prediction - Load baseline 1 pre-evaluation, include=FALSE}
survive_baseline1_eval_discrimination <-
  readRDS("data/survive_baseline1_eval_discrimination.rds")
```
...
survive_stack_eval_calibration |>
  saveRDS("data/survive_stack_eval_calibration.rds")
...
survive_stack_eval_discrimination |>
  saveRDS("data/survive_stack_eval_discrimination.rds")
```

```{r Survival prediction - Load stack pre-evaluation, include=FALSE}
survive_stack_eval_calibration <-
  readRDS("data/survive_stack_eval_calibration.rds")
survive_stack_eval_discrimination <-
  readRDS("data/survive_stack_eval_discrimination.rds")
```

Best model:

```{r Survival prediction - Show stack evaluation, echo=FALSE}
survive_stack_eval_discrimination |>
  select_best_auroc_lb_model() |>
  cat()
```

```{r Survival prediction - Stack calibration plots, include=FALSE}
survive_stack_eval_calibration_plot <-
  survive_stack_eval_calibration |>
  lapply(\(x) x$validation) |>
  lapply(arrange_calib_plot)
```

```{r Create figure-6, include=FALSE}
figure6 <-
  ggarrange(
    survive_stack_eval_calibration_plot$survive_stack_rr
    , survive_stack_eval_calibration_plot$survive_stack_rf
    , survive_stack_eval_calibration_plot$survive_stack_gbm
    , survive_stack_eval_calibration_plot$survive_stack_dnn
    , nrow = 1, ncol = 4
    , widths = c(5, 5, 5, 5)
    , labels = LETTERS[1:4]
  )
...
```{r figure-6, echo=FALSE, fig.height=6, fig.width=20}
figure6
```

Figure 6. Survival stack prediction model calibration plots using validation set: (A) RR; (B) RF; (C) GBM; and (D) DNN. Vertical dashed lines indicate the standard threshold.

```{r Survival prediction - Stack ROC curves, include=FALSE}
survive_stack_eval_discrimination_plot <-
  list(
    plot =
      list(
        data =
          survive_stack_eval_discrimination |>
          imap(
            ~ .x$validation$plot$data |>
              mutate(model = str_to_upper(str_remove_all(.y, "survive_stack_")))
          ) |>
          reduce(rbind)
      )
  ) |>
...
```{r Survival prediction - Stack DCAs, eval=FALSE, include=FALSE}
survive_stack_eval_decision <-
  survive_stack_eval_df_list[
    survive_stack_eval_calibration |>
      select_well_calibrated_model()
  ] |>
  lapply(\(x) lapply(lapply(x, select, -id), decision, seed = seed))

survive_stack_eval_decision |>
  saveRDS("data/survive_stack_eval_decision.rds")
```

```{r Survival prediction - Stack pre-DCAs, include=FALSE}
survive_stack_eval_decision <-
  readRDS("data/survive_stack_eval_decision.rds")
```

```{r Survival prediction - Stack decision curves, include=FALSE}
survive_stack_eval_decision_plot <-
  list(
    plot =
      list(
        data =
...
```{r Create figure-7, include=FALSE}
figure7 <-
  ggarrange(
    survive_stack_eval_decision_plot + theme(legend.position = "none")
    , survive_stack_eval_discrimination_plot
    , nrow = 1, ncol = 2
    , widths = c(4, 5)
    , labels = LETTERS[1:2]
...
```{r figure-7, echo=FALSE, fig.height=4, fig.width=9}
figure7
```

Figure 7. Survival stack prediction model decision and ROC curves using validation set. DNN, deep neural network; ROC, receiver operating characteristics; RR, ridge regression.
...
rop_severe_stack_eval_calibration |>
  saveRDS("data/rop_severe_stack_eval_calibration.rds")
...
rop_severe_stack_eval_discrimination |>
  saveRDS("data/rop_severe_stack_eval_discrimination.rds")
```

```{r Severe ROP prediction - Load stack pre-evaluation, include=FALSE}
rop_severe_stack_eval_calibration <-
  readRDS("data/rop_severe_stack_eval_calibration.rds")
rop_severe_stack_eval_discrimination <-
  readRDS("data/rop_severe_stack_eval_discrimination.rds")
```

Best model:

```{r Severe ROP prediction - Show stack evaluation, echo=FALSE}
rop_severe_stack_eval_discrimination |>
  select_best_auroc_lb_model() |>
  cat()
```

```{r Severe ROP prediction - Stack calibration plots, include=FALSE}
...
```{r Create figure-8, include=FALSE}
figure8 <-
  ggarrange(
    rop_severe_stack_eval_calibration_plot$rop_severe_stack_rr
    , rop_severe_stack_eval_calibration_plot$rop_severe_stack_rf
    , rop_severe_stack_eval_calibration_plot$rop_severe_stack_gbm
    , rop_severe_stack_eval_calibration_plot$rop_severe_stack_dnn
    , nrow = 1, ncol = 4
    , widths = c(5, 5, 5, 5)
    , labels = LETTERS[1:4]
  )
...
```{r figure-8, echo=FALSE, fig.height=6, fig.width=20}
figure8
```

Figure 8. Severe ROP stack prediction model calibration plots using validation set: (A) RR; (B) RF; (C) GBM; and (D) DNN. Vertical dashed lines indicate the standard threshold.

```{r Severe ROP prediction - Stack ROC curves, include=FALSE}
rop_severe_stack_eval_discrimination_plot <-
  list(
    plot =
      list(
        data =
          ...
          imap(
            ~ .x$validation$plot$data |>
              mutate(model = str_to_upper(str_remove_all(.y, "rop_severe_stack_")))
          ) |>
          reduce(rbind)
      )
  ) |>
...
```{r Severe ROP prediction - Stack DCAs, eval=FALSE, include=FALSE}
rop_severe_stack_eval_decision <-
  rop_severe_stack_eval_df_list[
    rop_severe_stack_eval_calibration |>
      select_well_calibrated_model()
  ] |>
...
rop_severe_stack_eval_decision |>
  saveRDS("data/rop_severe_stack_eval_decision.rds")
```

```{r Severe ROP prediction - Stack pre-DCAs, include=FALSE}
rop_severe_stack_eval_decision <-
  readRDS("data/rop_severe_stack_eval_decision.rds")
```

```{r Severe ROP prediction - Stack decision curves, include=FALSE}
rop_severe_stack_eval_decision_plot <-
  list(
    plot =
      list(
        data =
...
```{r Create figure-9, include=FALSE}
figure9 <-
  ggarrange(
    rop_severe_stack_eval_decision_plot + theme(legend.position = "none")
    , rop_severe_stack_eval_discrimination_plot
    , nrow = 1, ncol = 2
    , widths = c(4, 5)
    , labels = LETTERS[1:2]
...
```{r figure-9, echo=FALSE, fig.height=4, fig.width=9}
figure9
```

Figure 9. Severe ROP stack prediction model decision and ROC curves using validation set. DNN, deep neural network; GBM, gradient boosting machine; RF, random forest; ROC, receiver operating characteristics; RR, ridge regression.
...
bpd_stack_eval_calibration |>
  saveRDS("data/bpd_stack_eval_calibration.rds")
...
bpd_stack_eval_discrimination |>
  saveRDS("data/bpd_stack_eval_discrimination.rds")
```

```{r BPD prediction - Load stack pre-evaluation, include=FALSE}
bpd_stack_eval_calibration <-
  readRDS("data/bpd_stack_eval_calibration.rds")
bpd_stack_eval_discrimination <-
  readRDS("data/bpd_stack_eval_discrimination.rds")
```

Best model:

```{r BPD prediction - Show stack evaluation, echo=FALSE}
bpd_stack_eval_discrimination |>
  select_best_auroc_lb_model() |>
  cat()
```

```{r BPD prediction - Stack calibration plots, include=FALSE}
bpd_stack_eval_calibration_plot <-
  bpd_stack_eval_calibration |>
  lapply(\(x) x$validation) |>
  lapply(arrange_calib_plot)
```

```{r Create figure-10, include=FALSE}
figure10 <-
  ggarrange(
    bpd_stack_eval_calibration_plot$bpd_stack_rr
    , bpd_stack_eval_calibration_plot$bpd_stack_rf
    , bpd_stack_eval_calibration_plot$bpd_stack_gbm
    , bpd_stack_eval_calibration_plot$bpd_stack_dnn
    , nrow = 1, ncol = 4
    , widths = c(5, 5, 5, 5)
    , labels = LETTERS[1:4]
  )
...
```{r figure-10, echo=FALSE, fig.height=6, fig.width=20}
figure10
```

Figure 10. BPD stack prediction model calibration plots using validation set: (A) RR; (B) RF; (C) GBM; and (D) DNN. Vertical dashed lines indicate the standard threshold.

```{r BPD prediction - Stack ROC curves, include=FALSE}
bpd_stack_eval_discrimination_plot <-
  list(
    plot =
      list(
        data =
          ...
          imap(
            ~ .x$validation$plot$data |>
              mutate(model = str_to_upper(str_remove_all(.y, "bpd_stack_")))
          ) |>
          reduce(rbind)
      )
  ) |>
...
```{r BPD prediction - Stack DCAs, eval=FALSE, include=FALSE}
bpd_stack_eval_decision <-
  bpd_stack_eval_df_list[
    bpd_stack_eval_calibration |>
      select_well_calibrated_model()
  ] |>
...
bpd_stack_eval_decision |>
  saveRDS("data/bpd_stack_eval_decision.rds")
```

```{r BPD prediction - Stack pre-DCAs, include=FALSE}
bpd_stack_eval_decision <-
  readRDS("data/bpd_stack_eval_decision.rds")
```

```{r BPD prediction - Stack decision curves, include=FALSE}
bpd_stack_eval_decision_plot <-
  list(
    plot =
      list(
        data =
...
```{r Create figure-11, include=FALSE}
figure11 <-
  ggarrange(
    bpd_stack_eval_decision_plot + theme(legend.position = "none")
    , bpd_stack_eval_discrimination_plot
    , nrow = 1, ncol = 2
    , widths = c(4, 5)
    , labels = LETTERS[1:2]
...
```{r figure-11, echo=FALSE, fig.height=4, fig.width=9}
figure11
```

Figure 11. BPD stack prediction model decision and ROC curves using validation set. DNN, deep neural network; GBM, gradient boosting machine; RF, random forest; ROC, receiver operating characteristics.
...
bpd_moderate_severe_stack_eval_calibration |>
  saveRDS("data/bpd_moderate_severe_stack_eval_calibration.rds")
...
bpd_moderate_severe_stack_eval_discrimination |>
  saveRDS("data/bpd_moderate_severe_stack_eval_discrimination.rds")
```

```{r Mod/sev BPD prediction - Load stack pre-evaluation, include=FALSE}
bpd_moderate_severe_stack_eval_calibration <-
  readRDS("data/bpd_moderate_severe_stack_eval_calibration.rds")
bpd_moderate_severe_stack_eval_discrimination <-
  readRDS("data/bpd_moderate_severe_stack_eval_discrimination.rds")
```

Best model:

```{r Mod/sev BPD prediction - Show stack evaluation, echo=FALSE}
bpd_moderate_severe_stack_eval_discrimination |>
  select_best_auroc_lb_model() |>
  cat()
```

```{r Mod/sev BPD prediction - Stack calibration plots, include=FALSE}
bpd_moderate_severe_stack_eval_calibration_plot <-
  bpd_moderate_severe_stack_eval_calibration |>
  lapply(\(x) x$validation) |>
  lapply(arrange_calib_plot)
```

```{r Create figure-12, include=FALSE}
figure12 <-
  ggarrange(
    bpd_moderate_severe_stack_eval_calibration_plot$bpd_moderate_severe_stack_rr
    , bpd_moderate_severe_stack_eval_calibration_plot$bpd_moderate_severe_stack_rf
    , bpd_moderate_severe_stack_eval_calibration_plot$bpd_moderate_severe_stack_gbm
    , bpd_moderate_severe_stack_eval_calibration_plot$bpd_moderate_severe_stack_dnn
    , nrow = 1, ncol = 4
    , widths = c(5, 5, 5, 5)
    , labels = LETTERS[1:4]
  )
...
```{r figure-12, echo=FALSE, fig.height=6, fig.width=20}
figure12
```

Figure 12. Moderate/severe BPD stack prediction model calibration plots using validation set: (A) RR; (B) RF; (C) GBM; and (D) DNN. Vertical dashed lines indicate the standard threshold.

```{r Mod/sev BPD prediction - Stack ROC curves, include=FALSE}
bpd_moderate_severe_stack_eval_discrimination_plot <-
  list(
    plot =
      list(
        data =
          ...
          imap(
            ~ .x$validation$plot$data |>
              mutate(
                model =
                  str_to_upper(str_remove_all(.y, "bpd_moderate_severe_stack_"))
              )
          ) |>
          reduce(rbind)
      )
...
```{r Mod/sev BPD prediction - Stack DCAs, eval=FALSE, include=FALSE}
bpd_moderate_severe_stack_eval_decision <-
  bpd_moderate_severe_stack_eval_df_list[
    bpd_moderate_severe_stack_eval_calibration |>
      select_well_calibrated_model()
  ] |>
...
bpd_moderate_severe_stack_eval_decision |>
  saveRDS("data/bpd_moderate_severe_stack_eval_decision.rds")
```

```{r Mod/sev BPD prediction - Stack pre-DCAs, include=FALSE}
bpd_moderate_severe_stack_eval_decision <-
  readRDS("data/bpd_moderate_severe_stack_eval_decision.rds")
```

```{r Mod/sev BPD prediction - Stack decision curves, include=FALSE}
bpd_moderate_severe_stack_eval_decision_plot <-
  list(
    plot =
      list(
        data =
...
```{r Create figure-13, include=FALSE}
figure13 <-
  ggarrange(
    bpd_moderate_severe_stack_eval_decision_plot + theme(legend.position = "none")
    , bpd_moderate_severe_stack_eval_discrimination_plot
    , nrow = 1, ncol = 2
    , widths = c(4, 5)
    , labels = LETTERS[1:2]
...
```{r figure-13, echo=FALSE, fig.height=4, fig.width=9}
figure13
```

Figure 13. Mod/sev BPD stack prediction model decision and ROC curves using validation set. DNN, deep neural network; GBM, gradient boosting machine; RF, random forest; ROC, receiver operating characteristics; RR, ridge regression.
...
```{r EUGR-HC prediction - Baseline 1 evaluation, eval=FALSE, include=FALSE}
eugr_hc_baseline2_eval_df_list <-
  list(
    eugr_hc_baseline2_rr = eugr_hc_baseline2_rr
  )

eugr_hc_baseline2_eval_discrimination <-
  eugr_hc_baseline2_eval_df_list |>
  lapply(\(x) lapply(lapply(x, select, -id), discrimination, seed = seed))

eugr_hc_baseline2_eval_discrimination |>
  saveRDS("data/eugr_hc_baseline2_eval_discrimination.rds")
```

```{r EUGR-HC prediction - Load baseline2 pre-evaluation, include=FALSE}
eugr_hc_baseline2_eval_discrimination <-
  readRDS("data/eugr_hc_baseline2_eval_discrimination.rds")
```
...
eugr_hc_stack1_eval_calibration |>
  saveRDS("data/eugr_hc_stack1_eval_calibration.rds")
...
eugr_hc_stack1_eval_discrimination |>
  saveRDS("data/eugr_hc_stack1_eval_discrimination.rds")
```

```{r EUGR-HC prediction - Load stack1 pre-evaluation, include=FALSE}
eugr_hc_stack1_eval_calibration <-
  readRDS("data/eugr_hc_stack1_eval_calibration.rds")
eugr_hc_stack1_eval_discrimination <-
  readRDS("data/eugr_hc_stack1_eval_discrimination.rds")
```

Best model:

```{r EUGR-HC prediction - Show stack1 evaluation, echo=FALSE}
eugr_hc_stack1_eval_discrimination |>
  select_best_auroc_lb_model() |>
  cat()
```

```{r EUGR-HC prediction - Stack1 calibration plots, include=FALSE}
eugr_hc_stack1_eval_calibration_plot <-
  eugr_hc_stack1_eval_calibration |>
  lapply(\(x) x$validation) |>
  lapply(arrange_calib_plot)
```

```{r Create figure-14, include=FALSE}
figure14 <-
  ggarrange(
    eugr_hc_stack1_eval_calibration_plot$eugr_hc_stack1_rr
    , eugr_hc_stack1_eval_calibration_plot$eugr_hc_stack1_rf
    , eugr_hc_stack1_eval_calibration_plot$eugr_hc_stack1_gbm
    , eugr_hc_stack1_eval_calibration_plot$eugr_hc_stack1_dnn
    , nrow = 1, ncol = 4
    , widths = c(5, 5, 5, 5)
    , labels = LETTERS[1:4]
  )
...
```{r figure-14, echo=FALSE, fig.height=6, fig.width=20}
figure14
```

Figure 14. EUGR-HC stack1 prediction model calibration plots using validation set: (A) RR; (B) RF; (C) GBM; and (D) DNN. Vertical dashed lines indicate the standard threshold.

```{r EUGR-HC prediction - Stack1 ROC curves, include=FALSE}
eugr_hc_stack1_eval_discrimination_plot <-
  list(
    plot =
      list(
        data =
          ...
          imap(
            ~ .x$validation$plot$data |>
              mutate(
                model =
                  str_to_upper(str_remove_all(.y, "eugr_hc_stack1_"))
              )
          ) |>
          reduce(rbind)
      )
...
```{r EUGR-HC prediction - Stack1 DCAs, eval=FALSE, include=FALSE}
eugr_hc_stack1_eval_decision <-
  eugr_hc_stack1_eval_df_list[
    eugr_hc_stack1_eval_calibration |>
      select_well_calibrated_model()
  ] |>
...
eugr_hc_stack1_eval_decision |>
  saveRDS("data/eugr_hc_stack1_eval_decision.rds")
```

```{r EUGR-HC prediction - Stack1 pre-DCAs, include=FALSE}
eugr_hc_stack1_eval_decision <-
  readRDS("data/eugr_hc_stack1_eval_decision.rds")
```

```{r EUGR-HC prediction - Stack1 decision curves, include=FALSE}
eugr_hc_stack1_eval_decision_plot <-
  list(
    plot =
      list(
        data =
...
```{r Create figure-15, include=FALSE}
figure15 <-
  ggarrange(
    eugr_hc_stack1_eval_decision_plot + theme(legend.position = "none")
    , eugr_hc_stack1_eval_discrimination_plot
    , nrow = 1, ncol = 2
    , widths = c(4, 5)
    , labels = LETTERS[1:2]
...
```{r figure-15, echo=FALSE, fig.height=4, fig.width=9}
figure15
```

Figure 15. EUGR-HC stack1 prediction model decision and ROC curves using validation set. DNN, deep neural network; GBM, gradient boosting machine; RF, random forest; ROC, receiver operating characteristics.
...
eugr_hc_stack_eval_calibration |>
  saveRDS("data/eugr_hc_stack_eval_calibration.rds")
...
eugr_hc_stack_eval_discrimination |>
  saveRDS("data/eugr_hc_stack_eval_discrimination.rds")
```

```{r EUGR-HC prediction - Load stack pre-evaluation, include=FALSE}
eugr_hc_stack_eval_calibration <-
  readRDS("data/eugr_hc_stack_eval_calibration.rds")
eugr_hc_stack_eval_discrimination <-
  readRDS("data/eugr_hc_stack_eval_discrimination.rds")
```

Best model:

```{r EUGR-HC prediction - Show stack evaluation, echo=FALSE}
eugr_hc_stack_eval_discrimination |>
  select_best_auroc_lb_model() |>
  cat()
```

```{r EUGR-HC prediction - Stack calibration plots, include=FALSE}
eugr_hc_stack_eval_calibration_plot <-
  eugr_hc_stack_eval_calibration |>
  lapply(\(x) x$validation) |>
  lapply(arrange_calib_plot)
```

```{r Create figure-16, include=FALSE}
figure16 <-
  ggarrange(
    eugr_hc_stack_eval_calibration_plot$eugr_hc_stack_rr
    , eugr_hc_stack_eval_calibration_plot$eugr_hc_stack_rf
    , eugr_hc_stack_eval_calibration_plot$eugr_hc_stack_gbm
    , eugr_hc_stack_eval_calibration_plot$eugr_hc_stack_dnn
    , nrow = 1, ncol = 4
    , widths = c(5, 5, 5, 5)
    , labels = LETTERS[1:4]
  )
...
```{r figure-16, echo=FALSE, fig.height=6, fig.width=20}
figure16
```

Figure 16. EUGR-HC stack prediction model calibration plots using validation set: (A) RR; (B) RF; (C) GBM; and (D) DNN. Vertical dashed lines indicate the standard threshold.

```{r EUGR-HC prediction - Stack ROC curves, include=FALSE}
eugr_hc_stack_eval_discrimination_plot <-
  list(
    plot =
      list(
        data =
          ...
          imap(
            ~ .x$validation$plot$data |>
              mutate(
                model =
                  str_to_upper(str_remove_all(.y, "eugr_hc_stack_"))
              )
          ) |>
          reduce(rbind)
      )
...
```{r EUGR-HC prediction - Stack DCAs, eval=FALSE, include=FALSE}
eugr_hc_stack_eval_decision <-
  eugr_hc_stack_eval_df_list[
    eugr_hc_stack_eval_calibration |>
      select_well_calibrated_model()
  ] |>
...
eugr_hc_stack_eval_decision |>
  saveRDS("data/eugr_hc_stack_eval_decision.rds")
```

```{r EUGR-HC prediction - Stack pre-DCAs, include=FALSE}
eugr_hc_stack_eval_decision <-
  readRDS("data/eugr_hc_stack_eval_decision.rds")
```

```{r EUGR-HC prediction - Stack decision curves, include=FALSE}
eugr_hc_stack_eval_decision_plot <-
  list(
    plot =
      list(
        data =
...
```{r Create figure-17, include=FALSE}
figure17 <-
  ggarrange(
    eugr_hc_stack_eval_decision_plot + theme(legend.position = "none")
    , eugr_hc_stack_eval_discrimination_plot
    , nrow = 1, ncol = 2
    , widths = c(4, 5)
    , labels = LETTERS[1:2]
...
```{r figure-17, echo=FALSE, fig.height=4, fig.width=9}
figure17
```

Figure 17. EUGR-HC stack prediction model decision and ROC curves using validation set. DNN, deep neural network; GBM, gradient boosting machine; RF, random forest; ROC, receiver operating characteristics; RR, ridge regression.
...
```{r EUGR-BW prediction - Baseline 1 evaluation, eval=FALSE, include=FALSE}
eugr_bw_baseline2_eval_df_list <-
  list(
    eugr_bw_baseline2_rr = eugr_bw_baseline2_rr
  )

eugr_bw_baseline2_eval_discrimination <-
  eugr_bw_baseline2_eval_df_list |>
  lapply(\(x) lapply(lapply(x, select, -id), discrimination, seed = seed))

eugr_bw_baseline2_eval_discrimination |>
  saveRDS("data/eugr_bw_baseline2_eval_discrimination.rds")
```

```{r EUGR-BW prediction - Load baseline2 pre-evaluation, include=FALSE}
eugr_bw_baseline2_eval_discrimination <-
  readRDS("data/eugr_bw_baseline2_eval_discrimination.rds")
```
...
eugr_bw_stack1_eval_calibration |>
  saveRDS("data/eugr_bw_stack1_eval_calibration.rds")
...
eugr_bw_stack1_eval_discrimination |>
  saveRDS("data/eugr_bw_stack1_eval_discrimination.rds")
```

```{r EUGR-BW prediction - Load stack1 pre-evaluation, include=FALSE}
eugr_bw_stack1_eval_calibration <-
  readRDS("data/eugr_bw_stack1_eval_calibration.rds")
eugr_bw_stack1_eval_discrimination <-
  readRDS("data/eugr_bw_stack1_eval_discrimination.rds")
```

Best model:

```{r EUGR-BW prediction - Show stack1 evaluation, echo=FALSE}
eugr_bw_stack1_eval_discrimination |>
  select_best_auroc_lb_model() |>
  cat()
```

```{r EUGR-BW prediction - Stack1 calibration plots, include=FALSE}
eugr_bw_stack1_eval_calibration_plot <-
  eugr_bw_stack1_eval_calibration |>
  lapply(\(x) x$validation) |>
  lapply(arrange_calib_plot)
```

```{r Create figure-18, include=FALSE}
figure18 <-
  ggarrange(
    eugr_bw_stack1_eval_calibration_plot$eugr_bw_stack1_rr
    , eugr_bw_stack1_eval_calibration_plot$eugr_bw_stack1_rf
    , eugr_bw_stack1_eval_calibration_plot$eugr_bw_stack1_gbm
    , eugr_bw_stack1_eval_calibration_plot$eugr_bw_stack1_dnn
    , nrow = 1, ncol = 4
    , widths = c(5, 5, 5, 5)
    , labels = LETTERS[1:4]
  )
...
```{r figure-18, echo=FALSE, fig.height=6, fig.width=20}
figure18
```

Figure 18. EUGR-BW stack1 prediction model calibration plots using validation set: (A) RR; (B) RF; (C) GBM; and (D) DNN. Vertical dashed lines indicate the standard threshold.

```{r EUGR-BW prediction - Stack1 ROC curves, include=FALSE}
eugr_bw_stack1_eval_discrimination_plot <-
  list(
    plot =
      list(
        data =
          ...
          imap(
            ~ .x$validation$plot$data |>
              mutate(
                model =
                  str_to_upper(str_remove_all(.y, "eugr_bw_stack1_"))
              )
          ) |>
          reduce(rbind)
      )
...
```{r EUGR-BW prediction - Stack1 DCAs, eval=FALSE, include=FALSE}
eugr_bw_stack1_eval_decision <-
  eugr_bw_stack1_eval_df_list[
    eugr_bw_stack1_eval_calibration |>
      select_well_calibrated_model()
  ] |>
...
eugr_bw_stack1_eval_decision |>
  saveRDS("data/eugr_bw_stack1_eval_decision.rds")
```

```{r EUGR-BW prediction - Stack1 pre-DCAs, include=FALSE}
eugr_bw_stack1_eval_decision <-
  readRDS("data/eugr_bw_stack1_eval_decision.rds")
```

```{r EUGR-BW prediction - Stack1 decision curves, include=FALSE}
eugr_bw_stack1_eval_decision_plot <-
  list(
    plot =
      list(
        data =
...
```{r Create figure-19, include=FALSE}
figure19 <-
  ggarrange(
    eugr_bw_stack1_eval_decision_plot + theme(legend.position = "none")
    , eugr_bw_stack1_eval_discrimination_plot
    , nrow = 1, ncol = 2
    , widths = c(4, 5)
    , labels = LETTERS[1:2]
...
```{r figure-19, echo=FALSE, fig.height=4, fig.width=9}
figure19
```

Figure 19. EUGR-BW stack1 prediction model decision and ROC curves using validation set. DNN, deep neural network; GBM, gradient boosting machine; RF, random forest; ROC, receiver operating characteristics; RR, ridge regression.
...
eugr_bw_stack_eval_calibration |>
  saveRDS("data/eugr_bw_stack_eval_calibration.rds")
...
eugr_bw_stack_eval_discrimination |>
  saveRDS("data/eugr_bw_stack_eval_discrimination.rds")
```

```{r EUGR-BW prediction - Load stack pre-evaluation, include=FALSE}
eugr_bw_stack_eval_calibration <-
  readRDS("data/eugr_bw_stack_eval_calibration.rds")
eugr_bw_stack_eval_discrimination <-
  readRDS("data/eugr_bw_stack_eval_discrimination.rds")
```

Best model:

```{r EUGR-BW prediction - Show stack evaluation, echo=FALSE}
eugr_bw_stack_eval_discrimination |>
  select_best_auroc_lb_model() |>
  cat()
```

```{r EUGR-BW prediction - Stack calibration plots, include=FALSE}
eugr_bw_stack_eval_calibration_plot <-
  eugr_bw_stack_eval_calibration |>
  lapply(\(x) x$validation) |>
  lapply(arrange_calib_plot)
```

```{r Create figure-20, include=FALSE}
figure20 <-
  ggarrange(
    eugr_bw_stack_eval_calibration_plot$eugr_bw_stack_rr
    , eugr_bw_stack_eval_calibration_plot$eugr_bw_stack_rf
    , eugr_bw_stack_eval_calibration_plot$eugr_bw_stack_gbm
    , eugr_bw_stack_eval_calibration_plot$eugr_bw_stack_dnn
    , nrow = 1, ncol = 4
    , widths = c(5, 5, 5, 5)
    , labels = LETTERS[1:4]
  )
...
```{r figure-20, echo=FALSE, fig.height=6, fig.width=20}
figure20
```

Figure 20. EUGR-BW stack prediction model calibration plots using validation set: (A) RR; (B) RF; (C) GBM; and (D) DNN. Vertical dashed lines indicate the standard threshold.

```{r EUGR-BW prediction - Stack ROC curves, include=FALSE}
eugr_bw_stack_eval_discrimination_plot <-
  list(
    plot =
      list(
        data =
          ...
          imap(
            ~ .x$validation$plot$data |>
              mutate(
                model =
                  str_to_upper(str_remove_all(.y, "eugr_bw_stack_"))
              )
          ) |>
          reduce(rbind)
      )
...
```{r EUGR-BW prediction - Stack DCAs, eval=FALSE, include=FALSE}
eugr_bw_stack_eval_decision <-
  eugr_bw_stack_eval_df_list[
    eugr_bw_stack_eval_calibration |>
      select_well_calibrated_model()
  ] |>
...
eugr_bw_stack_eval_decision |>
  saveRDS("data/eugr_bw_stack_eval_decision.rds")
```

```{r EUGR-BW prediction - Stack pre-DCAs, include=FALSE}
eugr_bw_stack_eval_decision <-
  readRDS("data/eugr_bw_stack_eval_decision.rds")
```

```{r EUGR-BW prediction - Stack decision curves, include=FALSE}
eugr_bw_stack_eval_decision_plot <-
  list(
    plot =
      list(
        data =
...
```{r Create figure-21, include=FALSE}
figure21 <-
  ggarrange(
    eugr_bw_stack_eval_decision_plot + theme(legend.position = "none")
    , eugr_bw_stack_eval_discrimination_plot
    , nrow = 1, ncol = 2
    , widths = c(4, 5)
    , labels = LETTERS[1:2]
...
```{r figure-21, echo=FALSE, fig.height=4, fig.width=9}
figure21
```

Figure 21. EUGR-BW stack prediction model decision and ROC curves using validation set. DNN, deep neural network; GBM, gradient boosting machine; RF, random forest; ROC, receiver operating characteristics; RR, ridge regression.
...
hearing_stack_eval_calibration |>
  saveRDS("data/hearing_stack_eval_calibration.rds")
...
hearing_stack_eval_discrimination |>
  saveRDS("data/hearing_stack_eval_discrimination.rds")
```

```{r Failed hearing prediction - Load stack pre-evaluation, include=FALSE}
hearing_stack_eval_calibration <-
  readRDS("data/hearing_stack_eval_calibration.rds")
hearing_stack_eval_discrimination <-
  readRDS("data/hearing_stack_eval_discrimination.rds")
```

Best model:

```{r Failed hearing prediction - Show stack evaluation, echo=FALSE}
hearing_stack_eval_discrimination |>
  select_best_auroc_lb_model() |>
  cat()
```

```{r Failed hearing prediction - Stack calibration plots, include=FALSE}
hearing_stack_eval_calibration_plot <-
  hearing_stack_eval_calibration |>
  lapply(\(x) x$validation) |>
  lapply(arrange_calib_plot)
```

```{r Create figure-22, include=FALSE}
figure22 <-
  ggarrange(
    hearing_stack_eval_calibration_plot$hearing_stack_rr
    , hearing_stack_eval_calibration_plot$hearing_stack_rf
    , hearing_stack_eval_calibration_plot$hearing_stack_gbm
    , hearing_stack_eval_calibration_plot$hearing_stack_dnn
    , nrow = 1, ncol = 4
    , widths = c(5, 5, 5, 5)
    , labels = LETTERS[1:4]
  )
...
```{r figure-22, echo=FALSE, fig.height=6, fig.width=20}
figure22
```

Figure 22. Failed hearing test stack prediction model calibration plots using validation set: (A) RR; (B) RF; (C) GBM; and (D) DNN. Vertical dashed lines indicate the standard threshold.

```{r Failed hearing prediction - Stack ROC curves, include=FALSE}
hearing_stack_eval_discrimination_plot <-
  list(
    plot =
      list(
        data =
          ...
          imap(
            ~ .x$validation$plot$data |>
              mutate(
                model =
                  str_to_upper(str_remove_all(.y, "hearing_stack_"))
              )
          ) |>
          reduce(rbind)
      )
...
```{r Failed hearing prediction - Stack DCAs, eval=FALSE, include=FALSE}
hearing_stack_eval_decision <-
  hearing_stack_eval_df_list[
    hearing_stack_eval_calibration |>
      select_well_calibrated_model()
  ] |>
...
hearing_stack_eval_decision |>
  saveRDS("data/hearing_stack_eval_decision.rds")
```

```{r Failed hearing prediction - Stack pre-DCAs, include=FALSE}
hearing_stack_eval_decision <-
  readRDS("data/hearing_stack_eval_decision.rds")
```

```{r Failed hearing prediction - Stack decision curves, include=FALSE}
hearing_stack_eval_decision_plot <-
  list(
    plot =
      list(
        data =
...
```{r Create figure-23, include=FALSE}
figure23 <-
  ggarrange(
    hearing_stack_eval_decision_plot + theme(legend.position = "none")
    , hearing_stack_eval_discrimination_plot
    , nrow = 1, ncol = 2
    , widths = c(4, 5)
    , labels = LETTERS[1:2]
...
```{r figure-23, echo=FALSE, fig.height=4, fig.width=9}
figure23
```

Figure 23. Failed hearing stack prediction model decision and ROC curves using validation set. DNN, deep neural network; GBM, gradient boosting machine; RF, random forest; ROC, receiver operating characteristics; RR, ridge regression.
...
```{r Chain back SHAP across stacked models- create, include=FALSE}
...
```{r Show threshold for survive baseline by train set, echo=FALSE}
th$survive_baseline1_rr
```
...
eval_plot$survive_baseline1_rr <-
  ...
  arrange_eval_plot(
    shap_beeswarm_plot_obj =
      stacked_ori_shap_feature_values$survive_baseline1_rr |>
      shap_beeswarm_plot(1, Inf, seed, 0.75)
    , threshold = th$survive_baseline1_rr
    , dc_xby = 0.1, dc_yby = 0.1, dc_ta_angle = -60
  )
```

```{r figure-24, echo=FALSE, fig.height=6.5, fig.width=10}
eval_plot$survive_baseline1_rr
```

Figure 24. Survival baseline prediction model evaluation using test set for: (A) calibration plot; (B) decision curve; (C) ROC curve; and (D) SHAP beeswarm plot. Vertical dashed line in panels A and B indicates the chosen threshold. ROC, receiver operating characteristics; SHAP, the Shapley additive explanation.
...
```{r Show threshold for survive stack by train set, echo=FALSE}
th$survive_stack_dnn
```

Specificity at the chosen threshold:

```{r Specificity at threshold for survive stack by train set, echo=FALSE}
survive_stack_eval_discrimination$survive_stack_dnn$validation$plot$data |>
  cbind(data.frame(th2 = th$survive_stack_dnn)) |>
  filter(th == th2) |>
  pull(tnr)
```
...
eval_plot$survive_stack_dnn <-
  ...
  arrange_eval_plot(
    shap_beeswarm_plot_obj =
      stacked_ori_shap_feature_values$survive_stack_dnn |>
      shap_beeswarm_plot(1, Inf, seed, 0.75)
...
```{r figure-25, echo=FALSE, fig.height=7.5, fig.width=10}
eval_plot$survive_stack_dnn
```

Figure 25. Survival stacked prediction model evaluation using test set for: (A) calibration plot; (B) decision curve; (C) ROC curve; and (D) SHAP beeswarm plot. Vertical dashed line in panels A and B indicates the chosen threshold. ROC, receiver operating characteristics; SHAP, the Shapley additive explanation.

```{r Create figure-26, include=FALSE}
figure26 <-
  survive_baseline1_eval_discrimination |>
  c(survive_stack_eval_discrimination) |>
  imap(
      ...
      mutate(
        outcome =
          str_remove_all(
            ...
            , paste0(
                "_baseline1|_stack|"
               , paste0(paste0('_', algorithms$algorithm), collapse = "|")
              )
          ) |>
          str_to_sentence()
        , type =
          ...
          str_to_sentence()
        , model =
          str_to_upper(str_remove_all(.y, "survive_|baseline1_|stack_"))
      )
  ) |>
  reduce(rbind) |>
  ...
  mutate(
    ...
    , type = factor(type, c("Baseline", "Stack"))
    , model = factor(model, rev(str_to_upper(algorithms$algorithm)))
  ) |>
  group_by(outcome, type) |>
  mutate(best = ifelse(lb == max(lb), "Yes", "No")) |>
  ungroup() |>
  ggplot(aes(model, estimate, color = best)) +
  geom_errorbar(aes(ymin = lb, ymax = ub), width = 0.2) +
  geom_point() +
  facet_grid(
    outcome + type ~ ., scales = "free_y", space = "free_y", switch = "y"
  ) +
  coord_flip() +
  xlab("Model") +
...
```{r figure-26, echo=FALSE, fig.height=1.5, fig.width=7}
figure26
```

Figure 26. Predictive performance of survive-targeting prediction models using validation set. Only eligible models are shown. DNN, deep neural network; RR, ridge regression.
...
```{r Show threshold for severe ROP stack by train set, echo=FALSE}
th$rop_severe_stack_rr
```

Specificity at the chosen threshold:

```{r Specificity at threshold for severe ROP stack by train set, echo=FALSE}
rop_severe_stack_eval_discrimination$rop_severe_stack_rr$validation$plot$data |>
  cbind(data.frame(th2 = th$rop_severe_stack_rr)) |>
  filter(th == th2) |>
  pull(tnr)
```
...
eval_plot$rop_severe_stack_rr <-
  ...
  arrange_eval_plot(
    shap_beeswarm_plot_obj =
      stacked_ori_shap_feature_values$rop_severe_stack_rr |>
      shap_beeswarm_plot(1, Inf, seed, 0.75)
...
```{r figure-27, echo=FALSE, fig.height=8, fig.width=10}
eval_plot$rop_severe_stack_rr
```

Figure 27. Severe ROP stacked prediction model evaluation using test set for: (A) calibration plot; (B) decision curve; (C) ROC curve; and (D) SHAP beeswarm plot. Vertical dashed line in panels A and B indicates the chosen threshold. ROC, receiver operating characteristics; SHAP, the Shapley additive explanation.

```{r Create figure-28, include=FALSE}
figure28 <-
  survive_baseline1_eval_discrimination |>
  c(survive_stack_eval_discrimination) |>
  c(rop_severe_stack_eval_discrimination) |>
  imap(
    ~ .x$validation$metrics |>
      mutate(
        outcome =
          ...
          str_to_sentence() |>
          str_replace_all("Rop_severe", "Severe ROP")
        , type =
          str_remove_all(
            .y
            , paste0(
                "survive_|rop_severe_|[:digit:]+|"
               , paste0(paste0('_', algorithms$algorithm), collapse = "|")
              )
          ) |>
          str_to_sentence()
        , model =
          str_to_upper(
            str_remove_all(.y, "survive_|rop_severe_|baseline1_|stack_")
          )
      )
  ) |>
  reduce(rbind) |>
...
```{r figure-28, echo=FALSE, fig.height=2.5, fig.width=7}
figure28
```

Figure 28. Predictive performance of severe ROP-targeting prediction models using validation set. Only eligible models are shown. DNN, deep neural network; ROP, retinopathy of prematurity; RR, ridge regression.
...
```{r Show threshold for BPD stack by train set, echo=FALSE}
th$bpd_stack_rf
```

Specificity at the chosen threshold:

```{r Specificity at threshold for BPD stack by train set, echo=FALSE}
bpd_stack_eval_discrimination$bpd_stack_rf$validation$plot$data |>
  cbind(data.frame(th2 = th$bpd_stack_rf)) |>
  filter(th == th2) |>
  pull(tnr)
```
...
eval_plot$bpd_stack_rf <-
  ...
  arrange_eval_plot(
    shap_beeswarm_plot_obj =
      stacked_ori_shap_feature_values$bpd_stack_rf |>
      shap_beeswarm_plot(1, Inf, seed, 0.75)
...
```{r figure-29, echo=FALSE, fig.height=7, fig.width=10}
eval_plot$bpd_stack_rf
```

Figure 29. BPD stacked prediction model evaluation using test set for: (A) calibration plot; (B) decision curve; (C) ROC curve; and (D) SHAP beeswarm plot. Vertical dashed line in panels A and B indicates the chosen threshold. ROC, receiver operating characteristics; SHAP, the Shapley additive explanation.

```{r Create figure-30, include=FALSE}
figure30 <-
  survive_baseline1_eval_discrimination |>
  c(survive_stack_eval_discrimination) |>
  c(bpd_stack_eval_discrimination) |>
  imap(
    ~ .x$validation$metrics |>
      mutate(
        outcome =
        ...
        , model =
          str_to_upper(
            str_remove_all(.y, "survive_|bpd_|baseline1_|stack_")
          )
      )
  ) |>
  reduce(rbind) |>
...
```{r figure-30, echo=FALSE, fig.height=2.5, fig.width=7}
figure30
```

Figure 30. Predictive performance of BPD-targeting prediction models using validation set. Only eligible models are shown. BPD, bronchopulmonary disease; DNN, deep neural network; ROP, retinopathy of prematurity; RR, ridge regression.
...
```{r Show threshold for moderate/severe BPD stack by train set, echo=FALSE}
th$bpd_moderate_severe_stack_rf
```

Specificity at the chosen threshold:

```{r Specificity at threshold for mod/sev BPD stack by train set, echo=FALSE}
bpd_moderate_severe_stack_eval_discrimination[[
    "bpd_moderate_severe_stack_rf"
  ]]$validation$plot$data |>
  cbind(data.frame(th2 = th$bpd_moderate_severe_stack_rf)) |>
  filter(th == th2) |>
  pull(tnr)
```
    ...
    ~ .x$validation$metrics |>
      mutate(
        outcome =
          ...
          str_to_sentence() |>
          str_replace_all("Bpd", "BPD") |>
          str_replace_all("Bpd_moderate_severe", "Moderate/severe BPD")
        , type =
          str_remove_all(
            .y
            , paste0(
                "survive_|bpd_|moderate_severe_|[:digit:]+|"
               , paste0(paste0('_', algorithms$algorithm), collapse = "|")
              )
          ) |>
          str_to_sentence()
        , model =
          str_to_upper(
            str_remove_all(
              .y, "survive_|bpd_|moderate_severe_|baseline1_|stack_"
            )
          )
      )
  ) |>
...
```{r figure-32, echo=FALSE, fig.height=3.5, fig.width=7}
figure32
```

Figure 32. Predictive performance of moderate/severe BPD-targeting prediction models using validation set. Only eligible models are shown. BPD, bronchopulmonary disease; DNN, deep neural network; ROP, retinopathy of prematurity; RR, ridge regression.
...
```{r Show threshold for EUGR-HC baseline by train set, echo=FALSE}
th$eugr_hc_baseline2_rr
```
...
eval_plot$eugr_hc_baseline2_rr <-
  ...
  arrange_eval_plot(
    shap_beeswarm_plot_obj =
      stacked_ori_shap_feature_values$eugr_hc_baseline2_rr |>
      shap_beeswarm_plot(1, Inf, seed, 0.75)
...
```{r figure-33, echo=FALSE, fig.height=6.5, fig.width=10}
eval_plot$eugr_hc_baseline2
```

Figure 33. EUGR-HC baseline prediction model evaluation using test set for: (A) calibration plot; (B) decision curve; (C) ROC curve; and (D) SHAP beeswarm plot. Vertical dashed line in panels A and B indicates the chosen threshold. ROC, receiver operating characteristics; SHAP, the Shapley additive explanation.
...
```{r Show threshold for EUGR-HC stack by train set, echo=FALSE}
th$eugr_hc_stack_rr
```

Specificity at the chosen threshold:

```{r Specificity at threshold for EUGR-HC stack by train set, echo=FALSE}
eugr_hc_stack_eval_discrimination$eugr_hc_stack_rr$validation$plot$data |>
  cbind(data.frame(th2 = th$eugr_hc_stack_rr)) |>
  filter(th == th2) |>
  pull(tnr)
```
...
eval_plot$eugr_hc_stack_rr <-
  ...
  arrange_eval_plot(
    shap_beeswarm_plot_obj =
      stacked_ori_shap_feature_values$eugr_hc_stack_rr |>
      shap_beeswarm_plot(1, Inf, seed, 0.75)
...
```{r figure-34, echo=FALSE, fig.height=8, fig.width=10}
eval_plot$eugr_hc_stack_rr
```

Figure 34. EUGR-HC stacked prediction model evaluation using test set for: (A) calibration plot; (B) decision curve; (C) ROC curve; and (D) SHAP beeswarm plot. Vertical dashed line in panels A and B indicates the chosen threshold. ROC, receiver operating characteristics; SHAP, the Shapley additive explanation.

```{r Create figure-35, include=FALSE}
figure35 <-
  survive_baseline1_eval_discrimination |>
  c(eugr_hc_baseline2_eval_discrimination) |>
  c(eugr_hc_stack1_eval_discrimination) |>
  c(eugr_hc_stack_eval_discrimination) |>
  imap(
    ~ .x$validation$metrics |>
      mutate(
        outcome =
          str_remove_all(
            .y
            , paste0(
                "_baseline|[:digit:]+|_stack|"
               , paste0(paste0('_', algorithms$algorithm), collapse = "|")
              )
          ) |>
          str_to_sentence() |>
          str_replace_all("Eugr_hc", "EUGR-HC")
        , type =
          str_remove_all(
            .y
            , paste0(
        ...
        , model =
          str_to_upper(
            str_remove_all(
              .y, "survive_|eugr_hc_|baseline1_|baseline2_|stack_|stack1_"
            )
          )
      )
  ) |>
  ...
  mutate(
    outcome = factor(outcome, unique(outcome))
    , type = factor(type, c("Baseline1", "Baseline2", "Stack1", "Stack"))
    , model = factor(model, rev(str_to_upper(algorithms$algorithm)))
  ) |>
  group_by(outcome, type) |>
  mutate(best = ifelse(lb == max(lb), "Yes", "No")) |>
...
```{r figure-35, echo=FALSE, fig.height=3.5, fig.width=7}
figure35
```

Figure 35. Predictive performance of EUGR-HC-targeting prediction models using validation set. Only eligible models are shown. DNN, deep neural network; EUGR-HC, extrauterine growth restriction by head circumference; ROP, retinopathy of prematurity; RR, ridge regression.
...
```{r Show threshold for EUGR-BW baseline by train set, echo=FALSE}
th$eugr_bw_baseline2_rr
```
...
eval_plot$eugr_bw_baseline2_rr <-
  ...
  arrange_eval_plot(
    shap_beeswarm_plot_obj =
      stacked_ori_shap_feature_values$eugr_bw_baseline2_rr |>
      shap_beeswarm_plot(1, Inf, seed, 0.75)
...
```{r figure-36, echo=FALSE, fig.height=6.5, fig.width=10}
eval_plot$eugr_bw_baseline2
```

Figure 36. EUGR-BW baseline prediction model evaluation using test set for: (A) calibration plot; (B) decision curve; (C) ROC curve; and (D) SHAP beeswarm plot. Vertical dashed line in panels A and B indicates the chosen threshold. ROC, receiver operating characteristics; SHAP, the Shapley additive explanation.
...
```{r Show threshold for EUGR-BW stack by train set, echo=FALSE}
th$eugr_bw_stack_rr
```

Specificity at the chosen threshold:

```{r Specificity at threshold for EUGR-BW stack by train set, echo=FALSE}
eugr_bw_stack_eval_discrimination$eugr_bw_stack_rr$validation$plot$data |>
  cbind(data.frame(th2 = th$eugr_bw_stack_rr)) |>
  filter(th == th2) |>
  pull(tnr)
```
...
eval_plot$eugr_bw_stack_rr <-
  ...
  arrange_eval_plot(
    shap_beeswarm_plot_obj =
      stacked_ori_shap_feature_values$eugr_bw_stack_rr |>
      shap_beeswarm_plot(1, Inf, seed, 0.75)
...
```{r figure-37, echo=FALSE, fig.height=8, fig.width=10}
eval_plot$eugr_bw_stack
```

Figure 37. EUGR-BW stacked prediction model evaluation using test set for: (A) calibration plot; (B) decision curve; (C) ROC curve; and (D) SHAP beeswarm plot. Vertical dashed line in panels A and B indicates the chosen threshold. ROC, receiver operating characteristics; SHAP, the Shapley additive explanation.

```{r Create figure-38, include=FALSE}
figure38 <-
  survive_baseline1_eval_discrimination |>
  c(eugr_bw_baseline2_eval_discrimination) |>
  c(eugr_bw_stack1_eval_discrimination) |>
  c(eugr_bw_stack_eval_discrimination) |>
  imap(
    ~ .x$validation$metrics |>
      mutate(
        outcome =
          ...
          str_to_sentence() |>
          str_replace_all("Eugr_bw", "EUGR-BW")
        , type =
          str_remove_all(
            .y
            , paste0(
        ...
        , model =
          str_to_upper(
            str_remove_all(
              .y, "survive_|eugr_bw_|baseline1_|baseline2_|stack_|stack1_"
            )
          )
      )
  ) |>
...
```{r figure-38, echo=FALSE, fig.height=3.5, fig.width=7}
figure38
```

Figure 38. Predictive performance of EUGR-HC-targeting prediction models using validation set. Only eligible models are shown. DNN, deep neural network; EUGR-BW, extrauterine growth restriction by body weight; RR, ridge regression.
...
```{r Show threshold for failed hearing stack by train set, echo=FALSE}
th$hearing_stack_rr
```

Specificity at the chosen threshold:

```{r Specificity at threshold for failed hearing stack by train set, echo=FALSE}
hearing_stack_eval_discrimination$hearing_stack_rr$validation$plot$data |>
  cbind(data.frame(th2 = th$hearing_stack_rr)) |>
  filter(th == th2) |>
  pull(tnr)
```
...
eval_plot$hearing_stack_rr <-
  ...
  arrange_eval_plot(
    shap_beeswarm_plot_obj =
      stacked_ori_shap_feature_values$hearing_stack_rr |>
      shap_beeswarm_plot(1, Inf, seed, 0.75)
...
```{r figure-39, echo=FALSE, fig.height=7.5, fig.width=10}
eval_plot$hearing_stack_rr
```

Figure 39. Failed hearing test stacked prediction model evaluation using test set for: (A) calibration plot; (B) decision curve; (C) ROC curve; and (D) SHAP beeswarm plot. Vertical dashed line in panels A and B indicates the chosen threshold. ROC, receiver operating characteristics; SHAP, the Shapley additive explanation.

```{r Create figure-40, include=FALSE}
figure40 <-
  survive_baseline1_eval_discrimination |>
  c(hearing_stack_eval_discrimination) |>
  imap(
    ~ .x$validation$metrics |>
      mutate(
        outcome =
          ...
          str_to_sentence() |>
          str_replace_all("Hearing", "Failed\nhearing test")
        , type =
          str_remove_all(
            .y
            , paste0(
                "survive_|hearing_|[:digit:]+|"
               , paste0(paste0('_', algorithms$algorithm), collapse = "|")
              )
          ) |>
          str_to_sentence()
        , model =
          str_to_upper(
            str_remove_all(
              .y, "survive_|hearing_|baseline1_|stack_"
            )
          )
      )
  ) |>
...
```{r figure-40, echo=FALSE, fig.height=3.5, fig.width=7}
figure40
```

Figure 40. Predictive performance of failed hearing test-targeting prediction models using validation set. Only eligible models are shown. DNN, deep neural network; RR, ridge regression.
...
```{r figure-41, echo=FALSE, fig.height=3.5, fig.width=7}
...
Figure 41. Performance for predicting survival at particular time using the chosen threshold. The best timing was selected according to TPR and PPV using validation set. PPV, positive predictive value or precision; TPR, true positive rate or sensitivity.

```{r figure-42, echo=FALSE, fig.height=5, fig.width=7}
mutate(cm_selected_time_summary_val, set = "Validation") |>
  rbind(mutate(cm_selected_time_summary, set = "Test")) |>
  timing_selection_plot("rop_severe", "t_14")
```

Figure 42. Performance for predicting severe ROP at particular time using the chosen threshold. The best timing was selected according to TPR, TNR, and NPV using validation set. NPV, negative predictive value; ROP, retinopathy of prematurity; TNR, true negative rate or specificity; TPR, true positive rate or sensitivity.

```{r figure-43, echo=FALSE, fig.height=5, fig.width=7}
mutate(cm_selected_time_summary_val, set = "Validation") |>
  rbind(mutate(cm_selected_time_summary, set = "Test")) |>
  timing_selection_plot("bpd", "t_28")
```

Figure 43. Performance for predicting BPD at particular time using the chosen threshold. The best timing was selected according to TPR, TNR, and NPV using validation set. BPD, bronchopulmonary disease; NPV, negative predictive value; TNR, true negative rate or specificity; TPR, true positive rate or sensitivity.

```{r figure-44, echo=FALSE, fig.height=5, fig.width=7}
mutate(cm_selected_time_summary_val, set = "Validation") |>
  rbind(mutate(cm_selected_time_summary, set = "Test")) |>
  timing_selection_plot("bpd_moderate_severe", "t_28")
```

Figure 44. Performance for predicting moderate-to-severe BPD at particular time using the chosen threshold. The best timing was selected according to TPR, TNR, and NPV using validation set. BPD, bronchopulmonary disease; NPV, negative predictive value; TNR, true negative rate or specificity; TPR, true positive rate or sensitivity.

```{r figure-45, echo=FALSE, fig.height=5, fig.width=7}
mutate(cm_selected_time_summary_val, set = "Validation") |>
  rbind(mutate(cm_selected_time_summary, set = "Test")) |>
  timing_selection_plot("eugr_hc", "t_28")
```

Figure 45. Performance for predicting EUGR-HC at particular time using the chosen threshold. The best timing was selected according to TPR, PPV, TNR, and NPV using validation set. EUGR-HC, extrauterine growth restriction by head circumference; NPV, negative predictive value; PPV, positive predictive value or precision; TNR, true negative rate or specificity; TPR, true positive rate or sensitivity.

```{r figure-46, echo=FALSE, fig.height=5, fig.width=7}
mutate(cm_selected_time_summary_val, set = "Validation") |>
  rbind(mutate(cm_selected_time_summary, set = "Test")) |>
  timing_selection_plot("eugr_bw", "t_14")
```

Figure 46. Performance for predicting EUGR-BW at particular time using the chosen threshold. The best timing was selected according to TPR and NPV using validation set. EUGR-BW, extrauterine growth restriction by body weight; NPV, negative predictive value; TPR, true positive rate or sensitivity.

```{r figure-47, echo=FALSE, fig.height=5, fig.width=7}
mutate(cm_selected_time_summary_val, set = "Validation") |>
  rbind(mutate(cm_selected_time_summary, set = "Test")) |>
  timing_selection_plot("hearing", "t_14")
```

Figure 47. Performance for predicting failed hearing test at particular time using the chosen threshold. The best timing was selected according to TPR, TNR, and NPV using validation set. NPV, negative predictive value; TNR, true negative rate or specificity; TPR, true positive rate or sensitivity.

```{r Create figure-48, include=FALSE}
figure48 <-
...
```{r figure-48, echo=FALSE, fig.height=5, fig.width=7}
figure48 |>
...
Figure 48. Predictive performance using independent test sets at particular time.
...
```{r figure-49, echo=FALSE, fig.height=2, fig.width=7}
...
Figure 49. Predictive performance of BPD prediction models in our study versus previous studies. *, Kwok TC et al. (2023) did not summarize the estimates; hence, each individual's c-statistics are shown (n=6); BPD, bronchopulmonary disease; CI, confidence interval; PI, prediction interval.
...
```{r figure-50, echo=FALSE, fig.height=1.5, fig.width=7}
...
Figure 50. Predictive performance of moderate/severe BPD prediction models in our study versus previous studies. BPD, bronchopulmonary disease; CI, confidence interval.

```{r figure-51, echo=FALSE, fig.height=1.5, fig.width=7}
cm_selected_time_test_bpd_moderate_severe_disc$metrics |>
  mutate(
    study = "Our study, 95% CI (internal validation)"
    , lb = estimate - ci
  ...
  rbind(
    read_csv(
      "inst/extdata/10.1001_jamanetworkopen.2021.35879.csv"
      , show_col_types = FALSE
    )
  ) |>
  rbind(
    read_csv(
      "inst/extdata/10.1038_s41372-022-01392-8.csv"
      , show_col_types = FALSE
    )
  ) |>
  mutate_at("study", \(x) factor(x, rev(unique(x)))) |>
...
Figure 51. Predictive performance of severe ROP prediction models in our study versus previous studies. *, neither 95% CI nor summarized estimate were reported in this review; CI, confidence interval; ROP, retinopathy of prematurity.

```{r figure-52, echo=FALSE, fig.height=10, fig.width=7}
rbind(
  stacked_ori_shap_feature_values$survive_stack_dnn |>
    mutate(target = "survival")
  , stacked_ori_shap_feature_values$rop_severe_stack_rr |>
    mutate(target = "rop_severe")
  , stacked_ori_shap_feature_values$bpd_stack_rf |>
    mutate(target = "bpd")
  , stacked_ori_shap_feature_values$bpd_moderate_severe_stack_rf |>
    mutate(target = "bpd_moderate_severe")
  , stacked_ori_shap_feature_values$eugr_hc_stack_rr |>
    mutate(target = "eugr_hc")
  , stacked_ori_shap_feature_values$eugr_bw_stack_rr |>
    mutate(target = "eugr_bw")
  , stacked_ori_shap_feature_values$hearing_stack_rr |>
    mutate(target = "hearing")
  ) |>
  filter(feature == "maternal_age") |>
  mutate(
    target = factor(target, unique(target))
    , feature_value_group =
      ifelse(
        feature_value < 20
        , "<20"
        , ifelse(
            ...
            , "35+"
            , paste0(
                floor(feature_value / 5) * 5
                , " - "
                , (floor(feature_value / 5) * 5) + 4
              )
          )
      ) |>
      factor(c("<20", paste0(seq(20, 30, 5), " - ", seq(20, 30, 5) + 4), "35+"))
  ) |>
  group_by(target, feature_value_group) |>
  summarize(
    avg = mean(shap_value)
    , ci = qnorm(0.975) * sd(shap_value) / sqrt(n())
    , .groups = "drop"
  ) |>
  mutate(lb = avg - ci, ub = avg + ci) |>
  ggplot(aes(factor(feature_value_group), avg)) +
  ...
  coord_flip() +
  xlab("") +
  ylab("SHAP value (95% CI)")
```

Figure 52. SHAP values of maternal age. CI, confidence interval.
...
```{r figure-53, echo=FALSE, fig.height=5, fig.width=7}
demo_trajectories |>
...
Figure 53. A case with ~50% truly-predicted survive and ~20% truly-predicted positives for other outcomes.
...
```{r figure-54, echo=FALSE, fig.height=3, fig.width=7}
demo_trajectories_plot[[1]]
```

Figure 54. Survival trajectories with ~90% truly and 1+ falsely predicted deaths and ~20% falsely predicted survive. Vertical dashed line indicates death age.

```{r figure-55, echo=FALSE, fig.height=5, fig.width=7}
demo_trajectories_plot[[2]]
```

Figure 55. Other outcome trajectories with ~20% truly and ~20% falsely predicted outcomes among ~50% truly predicted survive.
...
```{r Save deployment data, eval=FALSE, include=FALSE}
...
```{r Connection among ventilation predictors, include=FALSE}
multivar_reg_compiled_ventilation <-
  list(
    survive = survive_pred_features
    , rop_severe = rop_severe_pred_features
    , bpd = bpd_pred_features
  ...
  lapply(gather, type, used, -feature) |>
  imap(~ mutate(., target = .y)) |>
  reduce(rbind) |>
  filter(used == 1) |>
  select(-used) |>
  mutate(
    outcome =
      case_when(
        target == "survive" ~ "survive"
        , type %in% c("baseline", "baseline1") ~ "survive"
        , type == "baseline2" ~ target
        , type %in% c("stack", "stack1") ~ "survive"
        , target != "bpd_moderate_severe" & type == "stack2" ~ target
        , target == "bpd_moderate_severe" & type == "stack2" ~ "bpd"
        , target == "bpd_moderate_severe" & type == "stack3"
          ~ "bpd_moderate_severe"
      )
  ) |>
  left_join(
    multivar_reg_compiled |>
      left_join(
        multivar_reg_compiled |>
          select(outcome, variable, covariates) |>
          mutate(seq = seq(n())) |>
          separate_rows(covariates, sep = ", ") |>
          mutate(
            value = 1
            , covariates = ifelse(covariates == "", "no_covariate", covariates)
            , covariates = factor(covariates, unique(covariates))
          ) |>
          spread(covariates, value, fill = 0) |>
          arrange(seq) |>
          select(-seq, -no_covariate)
        , by = join_by(outcome, variable)
        , relationship = "many-to-many"
      ) |>
      select(-covariates) |>
      filter(str_detect(variable, "\\*")) |>
      mutate(feature = str_remove_all(variable, "\\*"))
    , by = join_by(outcome, feature)
  ) |>
  filter(str_detect(feature, "fio2_|resp_"))
```

```{r Create graph plot based on estimated direction, include=FALSE}
correlation_graph_plot <-
  unique(multivar_reg_compiled_ventilation$target) |>
  `names<-`(unique(multivar_reg_compiled_ventilation$target)) |>
  imap(
    ~ multivar_reg_compiled_ventilation |>
      filter(target == .x) |>
      select(
        -type, -target, -outcome, -variable, -term, -OR, -LB, -UB, -p.value
      ) |>
      gather(from, used, -feature) |>
      filter(used == 1) |>
      select(-used) |>
      rename(to = feature)
  )
...
correlation_graph_plot <-
  correlation_graph_plot[sapply(correlation_graph_plot, \(x) nrow(x) > 0)] |>
  lapply(graph_from_data_frame) |>
  imap(
    ~ ggnetwork(
        .x
        , layout = layout_as_tree(.x, root = c(), mode = "in", circular = TRUE)
        , arrow.gap = 0.075
      )
  ) |>
  imap(~ mutate(.x, outcome = .y)) |>
...
correlation_graph_plot <-
  correlation_graph_plot |>
  mutate(name = str_replace_all(name, "_cum", "\n_cum")) |>
  ggplot(aes(x, y, xend = xend, yend = yend, color = name, fill = name)) +
  geom_edges(
    arrow = arrow(length = unit(4, "pt"), type="closed")
    , curvature = 0.2, linewidth = 0.5, show.legend = FALSE
  ) +
  geom_nodelabel(
    aes(label = name), color = "white", size = 3, angle = 90
    , show.legend = FALSE
  ) +
  facet_wrap(~ outcome, ncol = 2) +
  scale_x_continuous(
    limits =
      c(min(correlation_graph_plot$x) - 0
...
```{r figure-56, echo=FALSE, fig.height=5, fig.width=10}
ggarrange(
  ggarrange(
    NULL
    , multivar_reg_compiled_ventilation |>
      mutate_at(c("target", "outcome"), \(x) factor(x, unique(x))) |>
      ggplot(aes(feature, OR)) +
      geom_hline(yintercept = 1, lty = 2) +
      geom_errorbar(aes(ymin = LB, ymax = UB), width = 0.2) +
      geom_point() +
      facet_grid(outcome ~ target, scales = "free", space = "free") +
      coord_flip() +
      xlab("") +
      scale_y_continuous(
        "OR (95% CI)", limits = c(0.85, 1.35), breaks = seq(0.75, 1.35, 0.25)
      ) +
      theme(
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)
    ...
    , correlation_graph_plot
    , nrow = 3, ncol = 1, heights = c(0.35, 3.5, 1.6)
  )
  , multivar_reg_compiled_ventilation |>
    select(-type, -variable, -term, -OR, -LB, -UB, -p.value) |>
    gather(feature2, covariate, -target, -outcome, -feature) |>
    filter(str_detect(feature2, "fio2_|resp_")) |>
    mutate_at(c("target", "outcome"), \(x) factor(x, unique(x))) |>
    mutate(covariate = factor(ifelse(covariate == 1, "Yes", "No"))) |>
    ggplot(aes(feature2, feature, fill = covariate)) +
    geom_tile(color = "white") +
    facet_grid(outcome ~ ., scales = "free", space = "free") +
    xlab("") +
    scale_fill_discrete("Adjustment") +
    theme(
      axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)
  ...
  , nrow = 1, ncol = 2, widths = c(7.5, 2.5)
)
```

Figure 56. Connection among ventilation predictors. CI, confidence interval; OR, odds ratio.
...
```{r Hyperparameters, include=FALSE}
hyperparameters <-
  read_csv("inst/extdata/hyperparameters.csv", show_col_types = FALSE)
```

```{r Hyperparameters on paper - create, include=FALSE}
table21 <-
  hyperparameters |>
  mutate(
    explainer =
      ifelse(duplicated(paste0(name, algorithm, explainer)), "", explainer)
    , algorithm =
      ifelse(duplicated(paste0(name, algorithm)), "", algorithm)
    , name =
      ifelse(duplicated(paste0(name)), "", name)
  ) |>
  rename_all(str_replace_all, "_", " ") |>
  rename_all(str_to_sentence)
```

```{r Hyperparameters on paper - show, echo=FALSE}
table21 |>
  kable(
    caption = "Table 21. Hyperparameters."
    , format = kable_format
  ) |>
  kable_classic() |>
  footnote(
  ...
  column_spec(seq(ncol(table21)), extra_css = "vertical-align:top;")
```

```{r Determine variables based on ID only and both ID and t, include=FALSE}
id_based_vars <-
  c(var_reg$independent, var_reg$covariates, var_reg$dependent) |>
  pbsapply(\(x) nrow(unique(select_at(modeval_data, c(var_reg$s, x)))))

id_based_vars <-
  names(id_based_vars)[
    id_based_vars == nrow(unique(select_at(modeval_data, var_reg$s)))
  ]

id_t_based_vars <-
  c(var_reg$independent, var_reg$covariates, var_reg$dependent) |>
  setdiff(id_based_vars)
```

```{r Modeling and evaluation data based on ID for int val, include=FALSE}
modeval_data_id_based_int <-
  modeval_data |>
  select_at(c(var_reg$s, id_based_vars)) |>
  unique() |>
  mutate(
    set =
      case_when(
        id %in% train_id ~ "Train"
        , id %in% val_id ~ "Validation"
        , id %in% test_id ~ "Test"
      ) |>
      factor(c("Train", "Validation", "Test", "Independent test"))
  )

modeval_data_id_based_int <- 
  modeval_data_id_based_int |>
  select_if(colnames(modeval_data_id_based_int) != var_reg$s)
```

```{r Modeling and evaluation data based on ID-t for int val, include=FALSE}
modeval_data_idt_based_int <-
  modeval_data |>
  select_at(c(var_reg$s, id_t_based_vars)) |>
  mutate(
    set =
      case_when(
        id %in% train_id ~ "Train"
...
modeval_data_idt_based_int <- 
  modeval_data_idt_based_int |>
  select_if(colnames(modeval_data_idt_based_int) != var_reg$s)
```

```{r Modeling and evaluation data based on ID for ext val, include=FALSE}
modeval_data_id_based_ext <-
  modeval_data_indep_test |>
  select_at(c(var_reg$s, id_based_vars)) |>
  unique() |>
  mutate(
...
modeval_data_id_based_ext <- 
  modeval_data_id_based_ext |>
  select_if(colnames(modeval_data_id_based_ext) != var_reg$s)
```

```{r Modeling and evaluation data based on ID-t for ext val, include=FALSE}
modeval_data_idt_based_ext <-
  modeval_data_indep_test |>
  select_at(id_t_based_vars) |>
  mutate(
    set =
      "Independent test" |>
      factor(c("Train", "Validation", "Test", "Independent test"))
...
```{r Variable type on paper - old, include=FALSE}
paper_old_type <-
  data.frame(
    old_type =
      modeval_data_id_based_int |>
      rbind(modeval_data_id_based_ext) |>
      select(-set) |>
      sapply(class)
    , new_type = NA
  ) |>
  ...
  rbind(
    data.frame(
        old_type =
          modeval_data_idt_based_int |>
          rbind(modeval_data_idt_based_ext) |>
          select(-set) |>
          sapply(class)
        , new_type = NA
      ) |>
...
```{r Variable type on paper - write, eval=FALSE, include=FALSE}
paper_old_type |>
  write_csv("inst/extdata/paper_old_type.csv")
```

```{r Variable type on paper - new, include=FALSE}
paper_new_type <-
  read_csv("inst/extdata/paper_new_type.csv", show_col_types = FALSE)
```

```{r Change variable type of modeval data based on ID, include=FALSE}
modeval_data_id_based <-
  modeval_data_id_based_int |>
  rbind(modeval_data_id_based_ext)
...
modeval_data_id_based <-
  ...
  lapply(
    ...
    modeval_data_id_based |>
      ...
      mutate_at(
        x
        , list(as.factor, as.numeric)[[
            ifelse(
              filter(paper_new_type, variable == x)$new_type == "numeric"
              , 2, 1
            )
          ]]
      )
  ...
  reduce(cbind) |>
  cbind(select(modeval_data_id_based, set))
```

```{r Change variable type of modeval data based on ID-t, include=FALSE}
modeval_data_idt_based <-
  modeval_data_idt_based_int |>
  rbind(modeval_data_idt_based_ext)
...
modeval_data_idt_based <-
  ...
  reduce(cbind) |>
  cbind(select(modeval_data_idt_based, set))
```

```{r Average and SD on paper per ID, include=FALSE}
avg_sd_data_paper_id <-
  select(modeval_data_id_based, set) |>
  cbind(select_if(modeval_data_id_based, is.numeric)) |>
  gather(variable, value, -set) |>
  filter(!is.na(value)) |>
  group_by(set, variable) |>
  mutate(N = n()) |>
  group_by(set, variable, N) |>
  summarize(
    avg = mean(value)
    , std = sd(value)
    , .groups = "drop"
...
```{r Average and SD on paper per ID-t, include=FALSE}
avg_sd_data_paper_idt <-
  select(modeval_data_idt_based, set) |>
  cbind(select_if(modeval_data_idt_based, is.numeric)) |>
  gather(variable, value, -set) |>
  filter(!is.na(value)) |>
  group_by(set, variable) |>
  mutate(N = n()) |>
...
```{r Num colname of modeval data on paper - old, include=FALSE}
paper_old_num_colname <-
  avg_sd_data_paper_id |>
  rbind(avg_sd_data_paper_idt) |>
  select(old_colname = variable) |>
  unique() |>
  mutate(new_colname = NA, group = NA, subgroup = NA)
```

```{r Num colname of modeval data on paper - write, eval=FALSE, include=FALSE}
paper_old_num_colname |>
  write_csv("inst/extdata/paper_old_num_colname.csv")
```

```{r Num colname of modeval data on paper - new, include=FALSE}
paper_new_num_colname <-
  read_csv("inst/extdata/paper_new_num_colname.csv", show_col_types = FALSE) |>
  mutate(value = "")
```

```{r Tidy up average and SD on paper, include=FALSE}
avg_sd_data_paper <-
  list(Subjects = avg_sd_data_paper_id, Trajectories = avg_sd_data_paper_idt) |>
  lapply(
    left_join
    , rename(paper_new_num_colname, variable = old_colname)
    , by = join_by(variable)
  ) |>
  lapply(mutate, variable = new_colname) |>
  lapply(select, -new_colname) |>
  imap(~ mutate(.x, level = .y)) |>
  reduce(rbind) |>
  mutate(statistics = "Avg. ± SD") |>
  select(set, level, variable, value, statistics, everything())
```

```{r Proportion on paper per ID, include=FALSE}
prop_n_data_paper_id <-
  select(modeval_data_id_based, set) |>
  cbind(
    select_if(modeval_data_id_based, \(x) !is.numeric(x)) |>
      select(-set) |>
      mutate_all(as.character)
  ) |>
  gather(variable, value, -set) |>
  group_by(set, variable) |>
  mutate(N = n()) |>
  group_by(set, variable, value, N) |>
  summarize(n = n(), .groups = "drop") |>
  mutate(p = round(n / N * 100, 0))
```

```{r Proportion on paper per ID-t, include=FALSE}
prop_n_data_paper_idt <-
  select(modeval_data_idt_based, set) |>
  cbind(
    select_if(modeval_data_idt_based, \(x) !is.numeric(x)) |>
      select(-set) |>
      mutate_all(as.character)
  ) |>
  gather(variable, value, -set) |>
...
```{r Cat col & cat of modeval data on paper - old, include=FALSE}
paper_old_cat_colname_cat <-
  prop_n_data_paper_id |>
  rbind(prop_n_data_paper_idt) |>
  select(old_colname = variable, old_cat = value) |>
  unique() |>
  mutate(new_colname = NA, new_cat = NA, group = NA, subgroup = NA) |>
  select(old_colname, new_colname, old_cat, new_cat, everything())
```

```{r Cat col & cat of modeval data on paper - write, eval=FALSE, include=FALSE}
paper_old_cat_colname_cat |>
  write_csv("inst/extdata/paper_old_cat_colname_cat.csv")
```

```{r Cat col & cat of modeval data on paper - new, include=FALSE}
paper_new_cat_colname_cat <-
  read_csv("inst/extdata/paper_new_cat_colname_cat.csv", show_col_types = FALSE)
```

```{r Tidy up proportion on paper, include=FALSE}
prop_n_data_paper <-
  list(Subjects = prop_n_data_paper_id, Trajectories = prop_n_data_paper_idt) |>
  lapply(
    left_join
    , rename(paper_new_cat_colname_cat, variable = old_colname, value = old_cat)
    , by = join_by(variable, value)
  ) |>
  lapply(mutate, variable = new_colname, value = new_cat) |>
  lapply(select, -new_colname, -new_cat) |>
  imap(~ mutate(.x, level = .y)) |>
  reduce(rbind) |>
  mutate(statistics = "n (%)") |>
  select(set, level, variable, value, statistics, everything()) |>
...
```{r Compile groups and subgroups of variables - unorder, include=FALSE}
group_subgroup_unordered <-
  select(avg_sd_data_paper, group, subgroup) |>
  rbind(select(prop_n_data_paper, group, subgroup)) |>
  unique() |>
  mutate(
    group =
      group |>
      factor(
        c("Prenatal", "At birth", "Daily", "Occasional", "Weekly"
          ,"Postmenstrual age 36 w", "Term-equivalent age", "Discharge"
        )
      )
  ) |>
...
```{r Compile groups and subgroups of variables - write, eval=FALSE, include=FALSE}
group_subgroup_unordered |>
  write_csv("inst/extdata/group_subgroup_unordered.csv")
```

```{r Compile groups and subgroups of variables - ordered, include=FALSE}
group_subgroup_ordered <-
  read_csv("inst/extdata/group_subgroup_ordered.csv", show_col_types = FALSE) |>
  mutate(
    group = factor(group, levels(group_subgroup_unordered$group))
    , subgroup = reorder(subgroup, order)
  )

group_subgroup_ordered <-
  paper_new_num_colname |>
  select(group, subgroup,  variable = new_colname, value ) |>
  rbind(
    paper_new_cat_colname_cat |>
      select(group, subgroup, variable = new_colname, value = new_cat)
  ) |>
  filter(!is.na(value)) |>
  mutate(
    group  = factor(group, levels(group_subgroup_ordered$group))
    , subgroup  = factor(subgroup, levels(group_subgroup_ordered$subgroup))
    , variable = factor(variable, unique(variable))
    , value = factor(value, c("", unique(value)[unique(value) != ""]))
  ) |>
  arrange(group, subgroup, variable, value)
```

```{r Summarize sample characteristics on paper, include=FALSE}
desc_stats_paper <-
  list(
    avg_sd_data_paper |>
      mutate_at(c("avg", "std"), round, 2) |>
      mutate(std = paste0("(", as.character(std), ")")) |>
      unite(avg_std, avg, std, sep=" ") |>
      rename(summary2 = avg_std)
    , prop_n_data_paper |>
      mutate(p = paste0("(", ifelse(p == 0, "≤0.5", as.character(p)), ")")) |>
      unite(n_p, n, p, sep = " ") |>
      rename(summary2 = n_p)
  ) |>
  reduce(rbind) |>
  mutate(
    level = ifelse(variable == "Age of death (day)", "Death subjects", level)
    , level = factor(level, c("Subjects", "Death subjects", "Trajectories"))
    , group  = factor(group, levels(group_subgroup_ordered$group))
    , subgroup  = factor(subgroup, levels(group_subgroup_ordered$subgroup))
    , variable = factor(variable, levels(group_subgroup_ordered$variable))
    , value = factor(value, levels(group_subgroup_ordered$value))
  ) |>
  select(level, group, subgroup, everything()) |>
  arrange(level, group, subgroup, variable, value)
...
desc_stats_paper <-
  ...
  left_join(
    desc_stats_paper |>
      ...
      unique() |>
      group_by(level) |>
      summarize(N2 = sum(N), .groups = "drop")
    , by = join_by(level)
  )

desc_stats_paper <-
  desc_stats_paper |>
  select(level, set, n = N, N2) |>
  unique() |>
  mutate(p = round(n / N2 * 100, 0)) |>
  mutate(p = paste0("(", as.character(p), ")")) |>
  unite(n_p, n, p, sep = " ") |>
  rename(summary2 = n_p) |>
  spread(set, summary2, fill = "0 (0)") |>
  mutate(
    group = factor("Total")
    , subgroup = factor("")
    , variable = factor("")
  ...
  select(level, group, subgroup, variable, value, statistics, everything()) |>
  rbind(
    desc_stats_paper |>
      select(-N) |>
      spread(set, summary2, fill = "0 (0)")
  ) |>
  mutate(level = paste0(level, " (N=", N2, ")")) |>
  select(-N2) |>
  mutate_at("level", \(x) factor(x, unique(x))) |>
  arrange(level, group, subgroup, variable, value)
```

```{r Sample characteristics on paper - create 1, include=FALSE}
table22 <-
  desc_stats_paper |>
  mutate_all(as.character) |>
  mutate(
    value =
      ifelse(
        variable
        %in% c(
          "Severe ROP", "BPD", "Moderate/severe BPD", "EUGR-BW", "EUGR-HC"
          , "Failed to pass hearing screening"
        )
        & value == "Missing"
        , paste0(value, "†")
        , value
  ...
  mutate(
    variable =
      ifelse(duplicated(paste0(level, group, subgroup, variable)), "", variable)
    , subgroup =
      ifelse(duplicated(paste0(level, group, subgroup)), "", subgroup)
    , group =
      ifelse(duplicated(paste0(level, group)), "", group)
    , level = ifelse(duplicated(level), "", level)
  ) |>
  rename_all(str_to_sentence) |>
  rename(` ` = Value, `  ` = Variable, `   ` = Subgroup, Variable = Group)
```

```{r Sample characteristics on paper - show 1, echo=FALSE}
table22 |>
  kable(
    caption = "Table 22. Sample characteristics."
    , format = kable_format
  ) |>
  kable_classic() |>
  footnote(
    paste0(
      "Total proportion may be >100% due to rounding. "
      , "*, by row; "
      , "†, A minority of the outcomes, except survival, was diagnosed before "
      , "death if any; "
      ...
      , "EUGR-BW, EUGR by body weight; "
      , "EUGR-HC, EUGR by head circumference; "
  ...
  column_spec(seq(ncol(table22)), extra_css = "vertical-align:top;")
```

```{r Sample key characteristics on paper - create, include=FALSE}
table23 <-
  desc_stats_paper |>
  mutate_all(as.character) |>
  mutate(
  ...
  mutate(
    variable = ifelse(group == "Total" & variable == "", "Total", variable)
  ) |>
  filter(
    variable
    %in% c(
      "Total"
      , "Maternal age (year)", "Maternal education level (college)"
      , "Socieconomic status"
      , "Gestational age at birth (week)", "Sex"
      , "Severe ROP", "BPD", "Moderate/severe BPD", "EUGR-BW", "EUGR-HC"
      , "Failed to pass hearing screening"
    )
  ) |>
  select(-level, -group, -subgroup) |>
  mutate(
    variable =
      ifelse(duplicated(paste0(variable)), "", variable)
  ) |>
  rename_all(str_to_sentence) |>
  rename(` ` = Value)
```

```{r Sample key characteristics on paper - show, echo=FALSE}
table23 |>
  kable(
    caption = "Table 23. Key characteristics of samples."
    , format = kable_format
  ) |>
  kable_classic() |>
  footnote(
  ...
  column_spec(seq(ncol(table23)), extra_css = "vertical-align:top;")
```

```{r Combine regression analysis for paper, include=FALSE}
paper_reg <-
  list(
    unadjusted =
      univar_reg |>
  ...
  lapply(mutate, OR = paste0(OR, " (", LB, ", ", UB, "; ", p.value, ")")) |>
  lapply(mutate, OR = ifelse(OR == " (, ; )", "1 (0, Inf; >0.05)", OR)) |>
  lapply(select, -LB, -UB, -p.value) |>
  imap(~ rename_at(.x, "OR", \(x) .y)) |>
  reduce(left_join, by = join_by(outcome, variable, term)) |>
  left_join(
    multivar_reg |>
      select(outcome, variable, covariates) |>
      separate_rows(covariates, sep = " \\+ ") |>
  ...
  mutate(
    adjusted = ifelse(sig.x == "", NA, adjusted)
    , sig.y = ifelse(sig.x == "", NA, sig.y)
    ,  covariates = ifelse(sig.x == "", "N/A", covariates)
  ) |>
  mutate_at(
    c("unadjusted", "sig.x", "adjusted", "sig.y"), \(x) ifelse(is.na(x), "", x)
  ) |>
  mutate(
    unadjusted = paste0(unadjusted, sig.x)
    , adjusted = paste0(adjusted, sig.y)
  ) |>
  select(-sig.x, -sig.y) |>
  left_join(
    reg_data |>
      select_if(is.factor) |>
      lapply(levels) |>
      imap(~ data.frame(variable = .y, reference = .x[1])) |>
      reduce(rbind)
    , by = join_by(variable)
  ) |>
  select(outcome, variable, term, reference, everything()) |>
  left_join(
    paper_new_cat_colname_cat |>
      select(outcome = old_colname, outcome2 = new_colname) |>
      unique()
    , by = join_by(outcome)
  ) |>
  mutate(outcome = outcome2) |>
  select(-outcome2) |>
  left_join(
    paper_new_num_colname |>
      mutate(value2 = "value") |>
      select(
        variable = old_colname, variable2 = new_colname
        , term = value2, term2 = value
      ) |>
      rbind(
        paper_new_cat_colname_cat |>
          select(
            variable = old_colname, variable2 = new_colname
            , term = old_cat, term2 = new_cat
          )
      ) |>
      mutate(
        term =
          ifelse(
            str_detect(variable2, "\\(scale\\)")
            & !str_detect(variable, "^birthtime")
            , "value", term
          )
        , term2 =
          ifelse(
  ...
  mutate(variable = variable2, term = term2) |>
  select(-variable2, -term2) |>
  left_join(
    paper_new_cat_colname_cat |>
      mutate(
        new_cat =
          ifelse(str_detect(old_colname, "^birthtime"), "others", new_cat)
      ) |>
      select(
        variable = new_colname, reference = old_cat, reference2 = new_cat
      ) |>
      unique()
    , by = join_by(variable, reference)
  ) |>
  mutate(reference = reference2) |>
  select(-reference2) |>
  mutate(
    term = ifelse(is.na(reference), term, paste0(term, " vs. ", reference))
  ) |>
  select(-reference) |>
  left_join(
    paper_new_num_colname |>
      select(covariates = old_colname, covariates2 = new_colname) |>
      rbind(
        paper_new_cat_colname_cat |>
          select(covariates = old_colname, covariates2 = new_colname)
      ) |>
  ...
  mutate(covariates = ifelse(is.na(covariates2), covariates, covariates2)) |>
  select(-covariates2) |>
  mutate(
    outcome = factor(outcome, unique(outcome))
    , variable = factor(variable, levels(group_subgroup_ordered$variable))
  ...
  left_join(
    group_subgroup_ordered |>
      select(group, subgroup, variable) |>
      unique()
    , by = join_by(variable)
  ) |>
  select(outcome, group, subgroup, variable, everything()) |>
  arrange(outcome, group, subgroup, variable) |>
  group_by(outcome, group, subgroup, variable, term, unadjusted, adjusted) |>
  summarize(
    covariates = paste0(unique(covariates), collapse = ", "), .groups = "drop"
  )
```

```{r Regression analysis on paper - create, include=FALSE}
table24 <-
  paper_reg |>
  mutate_all(as.character) |>
  mutate(
    variable =
      ifelse(duplicated(paste0(outcome, group, subgroup, variable)), "", variable)
    , subgroup =
      ifelse(duplicated(paste0(outcome, group, subgroup)), "", subgroup)
    , group =
      ifelse(duplicated(paste0(outcome, group)), "", group)
    , outcome = ifelse(duplicated(outcome), "", outcome)
  ) |>
  rename_all(str_to_sentence) |>
  rename(
    ` ` = Term, `  ` = Variable, `   ` = Subgroup, Variable = Group
    , `Unadjusted effect (95% CI)` = Unadjusted
    , `Adjusted effect (95% CI)` = Adjusted
  )
```

```{r Regression analysis on paper - show, echo=FALSE}
table24 |>
  kable(
    caption = "Table 24. Multiple regression analyses."
    , format = kable_format
  ) |>
  kable_classic() |>
  footnote(
    paste0(
      "Adjusted regression was conducted if the unadjusted one demonstrated "
      , "significant effect. "
      , "The adjusted regression was the same with unadjusted one when no "
      , "covariates was selected. "
      , "The effect was estimated by HR if at least one of variables was a "
      , "trajectory variable among independent and dependent variables and "
      , "covariates; otherwise, the effect was estimated by OR. "
      , "*, significant effect, i.e., either LB >1 or UB <1 and p-value ≤0.05; "
      , "AKI, acute kidney injury; "
      , "Avg., average; "
      , "BPD, bronchopulmonary dysplasia; "
  ...
  column_spec(seq(ncol(table24)), extra_css = "vertical-align:top;")
```

```{r Stacked originap SHAP feature values on paper, include=FALSE}
paper_stacked_ori_shap_feature_values <-
  stacked_ori_shap_feature_values |>
  lapply(
    left_join
    , paper_new_num_colname |>
      select(feature = old_colname, feature2 = new_colname) |>
      rbind(
        paper_new_cat_colname_cat |>
          select(feature = old_colname, feature2 = new_colname)
      ) |>
      unique() |>
      add_row(
        data.frame(feature = "expected_value", feature2 = "Expected value")
      ) |>
      add_row(data.frame(feature = "t", feature2 = "Postnatal day (day)"))
    , by = join_by(feature)
  ) |>
  lapply(mutate, feature = feature2) |>
  lapply(select, -feature2)
```

```{r Severe ROP eval on paper, include=FALSE}
paper_eval_rop_severe <-
  paper_arrange_eval_plot(
    calibration_plot = rop_severe_stack_eval_calibration_plot
    , decision_plot = rop_severe_stack_eval_decision_plot
    , discrimination_plot = rop_severe_stack_eval_discrimination_plot
    , threshold = th$rop_severe_stack_rr
    , shap_beeswarm_obj =
      paper_stacked_ori_shap_feature_values$rop_severe_stack_rr |>
      mutate(feature = str_wrap(feature, 30)) |>
      shap_beeswarm_plot(1, Inf, seed, 0.75, "Expected value")
    , outcome_name = "rop_severe"
    , model_name = "RR"
    , timing = "t_14"
    , ghi = c(11, 7.5, 1.5)
  )
...
```{r figure-57, echo=FALSE, fig.height=24, fig.width=14}
paper_eval_rop_severe
```

Figure 57. Severe ROP model evaluation for: (A) DNN calibration; (B) GBM calibration; (C) RF calibration; (D) RR calibration; (E) clinical utility by DCA; (F) discrimination by ROC; (G) explainability by SHAP; (H) time-specific discrimination by CM; and (I) generalization by CM using independent test set compared to test set. The values 0 and 1 in panels A to D indicate negative and positive outcome, respectively. A vertical dashed line in panel E indicates the selected threshold. A point in panel F indicates specificity and sensitivity of the best model using the selected threshold. The best timing for the best model was selected in panel I according to TPR, TNR, and NPV using validation set. AKI, acute kidney injury; AUC-ROC, the area under curve of ROC; CM, confusion matrix; CI, confidence interval; DNN, deep neural network; EUGR, extrauterine growth restriction; GBM, gradient boosting machine; hs-PDA, hemodynamically-significant patent ductus arteriosus; IMV, invasive mechanical ventilation; IVH; intraventricular hemorrhage; NPV, negative predictive value; PPV, positive predictive value or precision; PRBC, packed red blood cell; ROP, retinopathy of prematurity; RF, random forest; ROC, receiver operating characteristics; RR, ridge regression; SHAP, the Shapley additive explanation; SIP, spontaneous intestinal perforation; TNR, true negative rate or specificity; TPR, true positive rate or sensitivity/recall.

```{r Mod/sev BPD eval on paper, include=FALSE}
paper_eval_bpd_moderate_severe <-
  paper_arrange_eval_plot(
    calibration_plot = bpd_moderate_severe_stack_eval_calibration_plot
    , decision_plot = bpd_moderate_severe_stack_eval_decision_plot
    , discrimination_plot = bpd_moderate_severe_stack_eval_discrimination_plot
    , threshold = th$bpd_moderate_severe_stack_rf
    , shap_beeswarm_obj =
      paper_stacked_ori_shap_feature_values$bpd_moderate_severe_stack_rf |>
      mutate(feature = str_wrap(feature, 30)) |>
      shap_beeswarm_plot(1, Inf, seed, 0.75, "Expected value")
    , outcome_name = "bpd_moderate_severe"
    , model_name = "RF"
    , timing = "t_28"
    , ghi = c(11, 7.5, 1.5)
  )
...
```{r figure-58, echo=FALSE, fig.height=24, fig.width=14}
paper_eval_bpd_moderate_severe
```

Figure 58. Moderate/severe BPD model evaluation for: (A) DNN calibration; (B) GBM calibration; (C) RF calibration; (D) RR calibration; (E) clinical utility by DCA; (F) discrimination by ROC; (G) explainability by SHAP; (H) time-specific discrimination by CM; and (I) generalization by CM using independent test set compared to test set. The values 0 and 1 in panels A to D indicate negative and positive outcome, respectively. A vertical dashed line in panel E indicates the selected threshold. A point in panel F indicates specificity and sensitivity of the best model using the selected threshold. The best timing for the best model was selected in panel I according to TPR, TNR, and NPV using validation set. AUC-ROC, the area under curve of ROC; BPD, bronchopulmonary dysplasia; CM, confusion matrix; CI, confidence interval; DNN, deep neural network; GBM, gradient boosting machine; IMV, invasive mechanical ventilation; NPV, negative predictive value; PPV, positive predictive value or precision; RF, random forest; ROC, receiver operating characteristics; RR, ridge regression; SHAP, the Shapley additive explanation; TNR, true negative rate or specificity; TPR, true positive rate or sensitivity/recall.

```{r EUGR-HC eval on paper, include=FALSE}
paper_eval_eugr_hc <-
  paper_arrange_eval_plot(
    calibration_plot = eugr_hc_stack_eval_calibration_plot
    , decision_plot = eugr_hc_stack_eval_decision_plot
    , discrimination_plot = eugr_hc_stack_eval_discrimination_plot
    , threshold = th$eugr_hc_stack_rr
    , shap_beeswarm_obj =
      paper_stacked_ori_shap_feature_values$eugr_hc_stack_rr |>
      mutate(feature = str_wrap(feature, 30)) |>
      shap_beeswarm_plot(1, Inf, seed, 0.75, "Expected value")
    , outcome_name = "eugr_hc"
    , model_name = "RR"
    , timing = "t_28"
    , ghi = c(11, 7.5, 1.5)
  )
...
```{r figure-59, echo=FALSE, fig.height=24, fig.width=14}
paper_eval_eugr_hc
```

Figure 59. EUGR-HC model evaluation for: (A) DNN calibration; (B) GBM calibration; (C) RF calibration; (D) RR calibration; (E) clinical utility by DCA; (F) discrimination by ROC; (G) explainability by SHAP; (H) time-specific discrimination by CM; and (I) generalization by CM using independent test set compared to test set. The values 0 and 1 in panels A to D indicate negative and positive outcome, respectively. A vertical dashed line in panel E indicates the selected threshold. A point in panel F indicates specificity and sensitivity of the best model using the selected threshold. The best timing for the best model was selected in panel I according to TPR, PPV, TNR, and NPV using validation set. AKI, acute kidney injury; AUC-ROC, the area under curve of ROC; CM, confusion matrix; CI, confidence interval; DNN, deep neural network; EUGR, extrauterine growth restriction; EUGR-HC, EUGR by head circumference; GBM, gradient boosting machine; hs-PDA, hemodynamically-significant patent ductus arteriosus; IMV, invasive mechanical ventilation; IVH; intraventricular hemorrhage; NPV, negative predictive value; PPV, positive predictive value or precision; RF, random forest; ROC, receiver operating characteristics; RR, ridge regression; SHAP, the Shapley additive explanation; SIP, spontaneous intestinal perforation; TNR, true negative rate or specificity; TPR, true positive rate or sensitivity/recall.

```{r EUGR-BW eval on paper, include=FALSE}
paper_eval_eugr_bw <-
  paper_arrange_eval_plot(
    calibration_plot = eugr_bw_stack_eval_calibration_plot
    , decision_plot = eugr_bw_stack_eval_decision_plot
    , discrimination_plot = eugr_bw_stack_eval_discrimination_plot
    , threshold = th$eugr_bw_stack_rr
    , shap_beeswarm_obj =
      paper_stacked_ori_shap_feature_values$eugr_bw_stack_rr |>
      mutate(feature = str_wrap(feature, 30)) |>
      shap_beeswarm_plot(1, Inf, seed, 0.75, "Expected value")
    , outcome_name = "eugr_bw"
    , model_name = "RR"
    , timing = "t_14"
    , ghi = c(11, 7.5, 1.5)
  )
...
```{r figure-60, echo=FALSE, fig.height=24, fig.width=14}
paper_eval_eugr_bw
```

Figure 60. EUGR-BW BPD model evaluation for: (A) DNN calibration; (B) GBM calibration; (C) RF calibration; (D) RR calibration; (E) clinical utility by DCA; (F) discrimination by ROC; (G) explainability by SHAP; (H) time-specific discrimination by CM; and (I) generalization by CM using independent test set compared to test set. The values 0 and 1 in panels A to D indicate negative and positive outcome, respectively. A vertical dashed line in panel E indicates the selected threshold. A point in panel F indicates specificity and sensitivity of the best model using the selected threshold. The best timing for the best model was selected in panel I according to TPR and NPV using validation set. AKI, acute kidney injury; AUC-ROC, the area under curve of ROC; CM, confusion matrix; CI, confidence interval; DNN, deep neural network; EUGR, extrauterine growth restriction; EUGR-BW, EUGR by body weight; GBM, gradient boosting machine; hs-PDA, hemodynamically-significant patent ductus arteriosus; IMV, invasive mechanical ventilation; NPV, negative predictive value; PPV, positive predictive value or precision; RF, random forest; ROC, receiver operating characteristics; RR, ridge regression; SHAP, the Shapley additive explanation; SIP, spontaneous intestinal perforation; TNR, true negative rate or specificity; TPR, true positive rate or sensitivity/recall.

```{r Failed hearing eval on paper, include=FALSE}
paper_eval_hearing <-
  paper_arrange_eval_plot(
    calibration_plot = hearing_stack_eval_calibration_plot
    , decision_plot = hearing_stack_eval_decision_plot
    , discrimination_plot = hearing_stack_eval_discrimination_plot
    , threshold = th$hearing_stack_rr
    , shap_beeswarm_obj =
      paper_stacked_ori_shap_feature_values$hearing_stack_rr |>
      mutate(feature = str_wrap(feature, 30)) |>
      shap_beeswarm_plot(1, Inf, seed, 0.75, "Expected value")
    , outcome_name = "hearing"
    , model_name = "RR"
    , timing = "t_14"
    , ghi = c(11, 7.5, 1.5)
  )
...
```{r figure-61, echo=FALSE, fig.height=24, fig.width=14}
paper_eval_hearing
```

Figure 61. Failed-to-pass-hearing-screening model evaluation for: (A) DNN calibration; (B) GBM calibration; (C) RF calibration; (D) RR calibration; (E) clinical utility by DCA; (F) discrimination by ROC; (G) explainability by SHAP; (H) time-specific discrimination by CM; and (I) generalization by CM using independent test set compared to test set. The values 0 and 1 in panels A to D indicate negative and positive outcome, respectively. A vertical dashed line in panel E indicates the selected threshold. A point in panel F indicates specificity and sensitivity of the best model using the selected threshold. The best timing for the best model was selected in panel I according to TPR, TNR, and NPV using validation set. AKI, acute kidney injury; AUC-ROC, the area under curve of ROC; CM, confusion matrix; CI, confidence interval; DNN, deep neural network; GBM, gradient boosting machine; IMV, invasive mechanical ventilation; NPV, negative predictive value; PPV, positive predictive value or precision; RF, random forest; ROC, receiver operating characteristics; RR, ridge regression; SHAP, the Shapley additive explanation; TNR, true negative rate or specificity; TPR, true positive rate or sensitivity/recall.

```{r Predictor profile on paper, include=FALSE}
paper_predictor_profile <-
  paper_reg |>
  mutate_all(as.character) |>
  mutate_all(\(x) ifelse(x == "", NA, x)) |>
  mutate(estimate = ifelse(is.na(adjusted), unadjusted, adjusted)) |>
  select(outcome, group, subgroup, variable, term, estimate) |>
  separate(
    estimate, c("OR", "LB", "UB", "p_value", "sig"), sep = " \\(|, |; |\\)"
  ) |>
  mutate_at(
    c("OR", "LB", "UB", "p_value")
    , \(x) 
  ...
  mutate_at(c("OR", "LB", "UB", "p_value"), as.numeric) |>
  mutate_at(
    c("OR", "LB", "UB"), \(x) ifelse(x > 4, 4, ifelse(x <= 1/4, 1/4, x))
  ) |>
  mutate_at(c("OR", "LB", "UB"), log2) |>
  mutate_at("term", \(x) ifelse(is.na(x), "", x)) |>
  unite(variable_term, variable, term, sep = " - ") |>
  mutate(
    variable_term =
      variable_term |>
      str_remove_all(" - $| - Yes vs\\. No$| - No vs\\. Yes$") |>
      str_remove_all("\\([:alpha:]+\\s*[:alpha:]*\\)\\s*") |>
      str_replace_all("Cumulative exposure of ", "Cum. exp. of ") |>
      str_replace_all("body weight", "BW") |>
      str_replace_all("head circumference", "HC") |>
      str_replace_all("Socieconomic status", "SES")
  ) |>
  mutate(
    outcome =
      outcome |>
  ...
  mutate_at(
    c("outcome", "group", "subgroup")
    , \(x) factor(x, unique(x))
  ) |>
  mutate_at("variable_term", \(x) factor(x, rev(unique(x)))) |>
  group_by(variable_term) |>
  filter(sum(sig == "*") > 0) |>
  ungroup() |>
  ggplot(aes(outcome, variable_term, fill = OR)) +
  geom_tile(color = "white") +
  geom_text(aes(label = sig), size = 2.5) +
  facet_grid(
    group  + subgroup ~ ., scales = "free_y", space = "free_y", switch = "y"
  ) +
  scale_fill_gradientn(
    "Effect size"
		, colors = c("#71c837ff", "white", "#c83737ff")
		, breaks = c(-2, 0, 2)
		, labels = c("≤0.25", "1", "≥4")
	) +
  theme(
...
```{r ROC curve on paper, include=FALSE}
paper_roc0 <-
  list(
    plot =
      list(
...
paper_roc <-
  paper_roc0 |>
  paper_arrange_disc_plot(multiple = TRUE)
...
paper_roc <-
  ...
  geom_point(
    data =
      ...
      left_join(
        list(
            `Severe ROP` = th$rop_severe_stack_rr
            , `Moderate/severe\nBPD` = th$bpd_moderate_severe_stack_rf
            , `EUGR-HC` = th$eugr_hc_stack_rr
            , `EUGR-BW` = th$eugr_bw_stack_rr
            , `Failed to pass\nhearing screening` = th$hearing_stack_rr
          ) |>
          imap(~ data.frame(model = .y, selected_th = .x)) |>
          reduce(rbind) |>
          mutate(model = str_wrap(model, 20)) |>
          mutate_at("model", \(x) factor(x, unique(x)))
        , by = join_by(model)
      ) |>
      filter(th == selected_th)
    , size = 1
  ) +
  scale_color_manual(
    "Model"
    , values =
      c("#3737c8ff", "#71c837ff", "#c83737ff", "#37abc8ff", "#916f6fff")
  )
```

```{r Best timing for the best model on paper, include=FALSE}
paper_timing <-
  cm_selected_time_summary |>
  filter(!outcome %in% c("survive", "bpd")) |>
  ...
  timing_selection_filter() |>
  select(outcome, timing) |>
  unique() |>
  left_join(
    paper_new_cat_colname_cat |>
      select(outcome = old_colname, new_colname) |>
      unique()
    , by = join_by(outcome)
  ) |>
  mutate(outcome = new_colname) |>
  select(-new_colname) |>
  mutate_at(
    "outcome"
    , \(x)
...
paper_timing <-
  ...
  mutate(variable_type = "Outcome") |>
  rbind(
    paper_timing |>
      mutate(
        outcome = "Survival predictors"
        , variable_type = "Survival predictors"
      ) |>
      unique()
  ) |>
  rbind(
    paper_predictor_profile$data |>
      filter(sig == "*") |>
      filter(!outcome %in% c("Survival", "BPD")) |>
      filter(
        !variable_term
         %in% c(
           paper_predictor_profile$data |>
             filter(, sig == "*" & outcome == "Survival") |>
             pull(variable_term)
         )
      ) |>
      left_join(paper_timing, by = join_by(outcome)) |>
      arrange(group, subgroup, desc(variable_term)) |>
      select(variable_term, timing, subgroup) |>
      unique() |>
      rename(outcome = variable_term) |>
      rename(variable_type = subgroup)
  ) |>
  mutate(best = 1) |>
  mutate_at("outcome", \(x) factor(x, unique(x))) |>
  spread(outcome, best, fill = 0) |>
  right_join(
    expand.grid(
        variable_type =
          c("Outcome"
            , "Survival predictors"
            , levels(paper_predictor_profile$data$subgroup)
          )
        , timing = paste0("Day ", c(7, 14, 21, 28))
        , stringsAsFactors = FALSE
      )
    , by = join_by(variable_type, timing)
  ) |>
  mutate_at(
    "variable_type"
    , factor
  ...
  mutate_at("timing", factor, paste0("Day ", c(7, 14, 21, 28))) |>
  gather(outcome, best, -timing, -variable_type) |>
  mutate(best = ifelse(is.na(best), 0, best)) |>
  mutate_at("outcome", \(x) factor(x, unique(x))) |>
  mutate(
    outcome2 = ifelse(best == 1, as.character(outcome), "")
    , outcome2 = factor(outcome2, c(levels(outcome), ""))
    , outcome = factor(outcome, rev(levels(outcome)))
    , best = ifelse(best == 1, "✓", " ")
  )

paper_timing <-
  paper_timing |>
  group_by(variable_type, outcome) |>
  filter(sum(best != " ") > 0) |>
  ungroup() |>
  ggplot(aes(timing, outcome)) +
  geom_tile(color = "black", fill = "white") +
  geom_text(aes(label = best), size = 2)  +
  facet_grid(
    variable_type ~ ., scales = "free_y", space = "free_y", switch = "y"
  ) +
  theme(
    panel.border = element_blank()
    , panel.spacing = unit(0.25, "mm")
...
```{r Model perf on paper - plot, include=FALSE}
paper_performance_plot <-
  ggarrange(
    paper_predictor_profile
    , ggarrange(
        paper_roc
        , paper_timing
        , nrow = 1, ncol = 2, widths = c(3, 2.51181), labels = LETTERS[2:3]
        , font.label = list(size = 7)
      )
    , nrow = 2, ncol = 1, heights = c(3.5, 2), labels = c(LETTERS[1], "")
    , font.label = list(size = 7)
  )
```

```{r Model perf on paper - show, echo=FALSE, fig.height=5.5, fig.width=5.51181}
paper_performance_plot
```

Figure 62. The best prediction models: (A) predictors; (B) ROC curve using test set; and (C) best timing for each outcome and the required predictors. Panel A only shows predictors with significant effects on at least an outcome in univariate regression analysis. The color legend of panel A is capped at effect sizes of 0.25 and 4. The points in panel B indicate specificity and sensitivity of the best model using the selected threshold. *, significant effect, i.e., either LB >1 or UB <1 and p-value ≤0.05; AKI, acute kidney injury; BPD, bronchopulmonary dysplasia; CI, confidence interval; EUGR, extrauterine growth restriction; EUGR-BW, EUGR by body weight; EUGR-HC, EUGR by head circumference; hs-PDA, hemodynamically-significant patent ductus arteriosus; IMV, invasive mechanical ventilation; IVH, intraventricular hemorrhage; LB, lower bound of 95% CI; PRBC, packed red blood cell; ROP, retinopathy of prematurity; SD, standard deviation; SIP, spontaneous intestinal perforation; UB, upper bound of 95% CI.

```{r Ext pred pr on paper - plot, include=FALSE}
paper_ext_predictor_profile <-
  paper_reg |>
  mutate_all(as.character) |>
  mutate_all(\(x) ifelse(x == "", NA, x)) |>
  mutate(adjusted = ifelse(is.na(adjusted), unadjusted, adjusted)) |>
  select(
    outcome, group, subgroup, variable, term, unadjusted, adjusted, covariates
  ) |>
  gather(
    type, estimate, -outcome, -group, -subgroup, -variable, -term, -covariates
  ) |>
  separate(
    estimate, c("OR", "LB", "UB", "p_value", "sig"), sep = " \\(|, |; |\\)"
  ) |>
  ...
  filter(sum(sig == "*") > 0) |>
  ungroup() |>
  mutate_at("variable_term", \(x) factor(x, unique(x))) |>
  mutate(
    variable_term_num = as.numeric(variable_term)
  )

paper_ext_predictor_profile_covariates <-
  paper_ext_predictor_profile |>
  select(outcome = variable_term_num, variable_term, covariates) |>
  mutate_at("covariates", \(x) ifelse(x == "N/A", NA, x)) |>
  separate_rows(covariates, sep = ",") |>
  mutate_at("covariates", trimws)
...
paper_ext_predictor_profile_covariates <-
  ...
  inner_join(
    paper_ext_predictor_profile_covariates |>
      select(outcome, variable_term) |>
      unique() |>
      mutate_at("variable_term", as.character) |>
      mutate_at(
        "variable_term", \(x) ifelse(str_detect(x, " - "), x, paste0(x, " - "))
      ) |>
      separate(variable_term, c("covariates", "term"), sep = " - ") |>
      mutate_at("covariates", trimws) |>
      select(-term)
    , by = join_by(covariates)
  ) |>
...
paper_ext_predictor_profile_covariates <-
  ...
  right_join(
    expand.grid(
        variable_term =
          levels(paper_ext_predictor_profile_covariates$variable_term)
        , outcome = sort(unique(paper_ext_predictor_profile_covariates$outcome))
      )
    , by = join_by(variable_term, outcome)
  ) |>
  mutate(OR = 0, LB = 0, UB = 0, p_value = 1) |>
  select(-covariates) |>
  left_join(
    paper_ext_predictor_profile |>
      select(group, subgroup, variable_term, variable_term_num) |>
      unique()
    , by = join_by(variable_term)
  ) |>
...
paper_ext_predictor_profile <-
  ...
  rbind(paper_ext_predictor_profile_covariates) |>
  mutate_at("outcome", \(x) factor(x, unique(x))) |>
  mutate(
    variable_term = as.character(variable_term)
    , variable_term = paste0(variable_term, " - ", variable_term_num)
  ) |>
  mutate_at("variable_term", \(x) factor(x, rev(unique(x)))) |>
  mutate_at("type", str_to_sentence) |>
  mutate_at("type", \(x) factor(x, unique(x)))

paper_ext_predictor_profile_covariates <-
  paper_ext_predictor_profile |>
  filter(type == "Covariates") |>
  mutate_at("outcome", as.character) |>
...
paper_ext_predictor_profile_covariates <-
  ...
  mutate(
    ...
    , OR = ifelse(outcome == variable_term_num, 1, OR)
  ) |>
  left_join(
    paper_ext_predictor_profile_covariates |>
      select(variable_term2 = variable_term, outcome = variable_term_num) |>
      unique()
    , by = join_by(outcome)
  ) |>
  mutate(
    outcome =
      factor(
        str_remove_all(as.character(variable_term2), " - [:digit:]+$")
        , rev(str_remove_all(levels(variable_term2), " - [:digit:]+$"))
      )
  ) |>
  select(-variable_term2) |>
  group_by(variable_term) |>
  filter(sum(sig == "✓", na.rm = TRUE) > 0) |>
  ungroup() |>
  select(outcome, variable_term, OR, group, subgroup, sig, type) |>
  unique() |>
  ggplot(aes(outcome, variable_term, fill = OR)) +
...
paper_ext_predictor_profile <-
  list(
    effect_size = paper_ext_predictor_profile
    , covariates = paper_ext_predictor_profile_covariates
  ) |>
  imap(
    ~ .x +
      geom_text(aes(label = sig), size = 1.5, na.rm = TRUE) +
      list(
        facet_grid(
          group  + subgroup ~ type, scales = "free", space = "free"
          , switch = "y"
        )
        , facet_grid(
            group  + subgroup ~ ., scales = "free", space = "free"
            , switch = "y"
          )
      )[[ifelse(.y == "covariates", 2, 1)]] +
      scale_fill_gradientn(
        "Effect size"
    		, colors =
    		  list(
    		    c("#71c837ff", "white", "#c83737ff"), c("white", "lightgrey")
    		  )[[ifelse(.y == "covariates", 2, 1)]]
    		, breaks =
    		  list(
    		    c(-2, 0, 2), c(0, 1)
      ...
      theme(
        ...
        , axis.text.x =
          list(
            ...
            , element_text(angle = 35, hjust = 1, vjust = 1, size = 5)
          )[[ifelse(.y == "covariates", 2, 1)]]
        , axis.ticks.y = element_line(linewidth = 0.25)
        , axis.text.y = element_text(size = 5)
        , strip.background = element_blank()
        ...
        , strip.text.y.left =
          list(
            element_text(size = 5, angle = 0, hjust = 1)
            , element_blank()
          )[[ifelse(.y == "covariates", 2, 1)]]
        , strip.placement = "outside"
        , legend.title = element_text(size = 5)
        ...
        , legend.key.height = unit(0.35, "cm")
    	  , legend.key.width = unit(0.35, "cm")
        , legend.position = ifelse(.y == "covariates", "none", "right")
      )
  )

paper_ext_predictor_profile_covariates <- paper_ext_predictor_profile[[2]]
paper_ext_predictor_profile <- paper_ext_predictor_profile[[1]]
```

```{r Ext pred pr on paper - arrange, include=FALSE}
paper_ext_predictor_profile_plot <-
  ggarrange(
    paper_ext_predictor_profile
    , paper_ext_predictor_profile_covariates
    , nrow = 2, ncol = 1, heights = c(3.75, 3.25), labels = LETTERS[1:2]
    , font.label = list(size = 7)
  )
```

```{r Ext pred pr on paper - show, echo=FALSE, fig.height=7, fig.width=5.51181}
paper_ext_predictor_profile_plot
```

Figure 63. The selected predictors according to multivariate regression analysis: (A) effect size; and (B) covariates. Numbers in y-axes of panels A and B are provided to assist variable identification across both panels. Panel A only shows predictors with significant effects on at least an outcome in univariate regression analysis (unadjusted). The color legend of panel A is capped at effect sizes of 0.25 and 4. Panel B only shows predictors with at least one covariate. In panel B, a grey box indicates the same variable in both axes. If a variable was used as a covariate for any other variables, grey tiles are assigned to that variable. *, significant effect, i.e., either LB >1 or UB <1 and p-value ≤0.05; AKI, acute kidney injury; Avg., average; BPD, bronchopulmonary dysplasia; CI, confidence interval; EUGR, extrauterine growth restriction; EUGR-BW, EUGR by body weight; EUGR-HC, EUGR by head circumference; hs-PDA, hemodynamically-significant patent ductus arteriosus; IMV, invasive mechanical ventilation; IVF, in vitro fertilization; IVH, intraventricular hemorrhage; LB, lower bound of 95% CI; N/A, not applicable; NEC, necrotizing enterocolitis; PRBC, packed red blood cell; ROP, retinopathy of prematurity; SIP, spontaneous intestinal perforation; UB, upper bound of 95% CI.

```{r Calibration plot on paper, include=FALSE}
paper_calib <-
  list(
    metrics =
      list(
...
paper_calib_plot <-
  paper_calib$metrics |>
  select(-ci) |>
  mutate_at("term", \(x) factor(x, unique(x))) |>
  spread(term, estimate, fill = 0) |>
  right_join(
    paper_calib$dist$data |>
      mutate(obs = ifelse(obs == 0, "negative", "positive")) |>
      spread(obs, n, fill = 0)
    , by = join_by(model)
  ) |>
  mutate(obs = intercept + slope * pred) |>
  ggplot(aes(pred, obs, color = model)) +
  geom_abline(intercept = 0, slope = 1, linewidth = 0.5, lty = 2) +
  geom_path(linewidth = 0.5, na.rm = TRUE) +
  coord_equal() +
  scale_x_continuous(
...
paper_calib_plot <-
  ...
  geom_point(
    data =
      paper_calib_plot$data |>
      select(model, intercept, slope) |>
      unique() |>
      left_join(
        list(
            `Severe ROP` = th$rop_severe_stack_rr
          ...
          imap(~ data.frame(model = .y, pred = .x)) |>
          reduce(rbind) |>
          mutate(model = str_wrap(model, 20)) |>
          mutate_at("model", \(x) factor(x, unique(x)))
        , by = join_by(model)
...
paper_calib_dist <-
  c("#3737c8ff", "#71c837ff", "#c83737ff", "#37abc8ff", "#916f6fff") |>
  `names<-`(levels(paper_calib$dist$data$model)) |>
  imap(
    ~ filter(paper_calib$dist$data, model == .y) |>
      ggplot(aes(pred, n)) +
      geom_col(fill = .x, na.rm = TRUE) +
      ...
      scale_y_continuous("Freq. (n)", n.breaks = 3, trans = "log10") +
      theme(
        axis.title.x =
          list(
            element_blank()
            ...
            which(levels(paper_calib$dist$data$model) %in% .y)
            == length(levels(paper_calib$dist$data$model))
            , 2, 1
          )]]
        , axis.text.x =
          list(
...
paper_calib_dist <-
  ggarrange(
    plotlist = paper_calib_dist
    , nrow = length(paper_calib_dist), ncol = 1, heights = c(rep(1, 4), 1.5)
  )
```

```{r Decision curve on paper, include=FALSE}
paper_dca <-
  list(
    data =
      list(
    ...
    , prevalence =
      ...
      imap(~ data.frame(prevalence = .x, model = .y)) |>
      reduce(rbind) |>
      mutate(model = str_wrap(model, 20)) |>
      mutate_at("model", \(x) factor(x, unique(x)))
  ) |>
  reduce(left_join, by = join_by(model)) |>
  mutate(treat_none = 0, treat_all = prevalence - 1 * th) |>
  mutate(ref_nb = ifelse(treat_all > treat_none , treat_all, treat_none)) |>
  mutate(residual_nb = nb - ref_nb) |>
...
```{r Evaluation on paper - plot, include=FALSE}
paper_evaluation_plot <-
    ggarrange(
    ggarrange(
      ggarrange(
        ggarrange(
          paper_calib_plot + theme(legend.position = "none"), NULL
          , nrow = 1, ncol = 2, widths = c(2.125, 0.125)
        )
        , paper_calib_dist
        , nrow = 2, ncol = 1, heights = c(1.75, 2.5)
      )
      , ggarrange(
          NULL, paper_dca + theme(legend.position = "none"), NULL
          , nrow = 1, ncol = 3, widths = c(0.2, 1.85, 0.2)
        )
      , paper_roc +
        theme(
      ...
      , nrow = 3, ncol = 1, heights = c(1.75 + 2.5, 0.75, 1.75 + 0.65)
      , labels = LETTERS[1:3], font.label = list(size = 7)
    )
    , ggarrange(
        paper_stacked_ori_shap_feature_values$rop_severe_stack_rr |>
          filter(str_detect(feature, "^Gest|^P[:alpha:]+ s|^Leu")) |>
          mutate(feature = str_wrap(feature, 30)) |>
          mutate(model = "Severe ROP") |>
          paper_shap_beeswarm_plot(
            1, Inf, seed, 0.75, "Expected value"
            , point_size = 0.75, colorbar_height = 3
          )
        , paper_stacked_ori_shap_feature_values$bpd_moderate_severe_stack_rf |>
          filter(str_detect(feature, "IMV|O2 [:alpha:]+ c")) |>
          mutate(feature = str_wrap(feature, 30)) |>
          mutate(model = "Moderate/severe BPD") |>
          paper_shap_beeswarm_plot(
            1, Inf, seed, 0.75, "Expected value"
            , point_size = 0.75, colorbar_height = 3
          )
        , paper_stacked_ori_shap_feature_values$eugr_hc_stack_rr |>
          filter(str_detect(feature, "^Gest|^Throm|^Inot")) |>
          mutate(feature = str_wrap(feature, 30)) |>
          mutate(model = "EUGR-HC") |>
          paper_shap_beeswarm_plot(
            1, Inf, seed, 0.75, "Expected value"
            , point_size = 0.75, colorbar_height = 3
          )
        , paper_stacked_ori_shap_feature_values$eugr_bw_stack_rr |>
          filter(str_detect(feature, "^Throm|expander|^Gest")) |>
          mutate(feature = str_wrap(feature, 30)) |>
          mutate(model = "EUGR-BW") |>
          paper_shap_beeswarm_plot(
            1, Inf, seed, 0.75, "Expected value"
            , point_size = 0.75, colorbar_height = 3
          )
        , paper_stacked_ori_shap_feature_values$hearing_stack_rr |>
          filter(str_detect(feature, "^Gest|^Throm|feeding")) |>
          mutate(feature = str_wrap(feature, 30)) |>
          mutate(model = "Failed to pass hearing screening") |>
          paper_shap_beeswarm_plot(
            1, Inf, seed, 0.75, "Expected value"
            , point_size = 0.75, colorbar_height = 3
          )
        , paper_timing
        , nrow = 6, ncol = 1, heights = c(0.98, 1.21, rep(0.98, 3), 2.25)
        , labels = c(LETTERS[4], rep("", 4), LETTERS[5])
        , font.label = list(size = 7)
      )
    , nrow = 1, ncol = 2, widths = c(2.25, 3.26181)
  )
```

```{r Evaluation on paper - show, echo=FALSE, fig.height=7.4, fig.width=5.51181}
paper_evaluation_plot
```

Figure 64. Evaluation of the best prediction models for: (A) calibration using test set; (B) clinical utility by decision curves using test set; (C) discrimination by ROC curves using test set; (D) explainability by SHAP beeswarm plots; and (E) best timing for each outcome and the required predictors. The color legend of panel C applies for panels A to C. The values 0 and 1 in panel A indicate negative and positive outcome, respectively. The points in panels A, B, and C respectively indicate true probabilities, residual net benefits, and specificities and sensitivities of the best model using the selected threshold. Panel D only shows predictors with the top-3 highest maximum SHAP values, as shown by numbers after the predictor names. AKI, acute kidney injury; BPD, bronchopulmonary dysplasia; EUGR, extrauterine growth restriction; EUGR-BW, EUGR by body weight; EUGR-HC, EUGR by head circumference; hs-PDA, hemodynamically-significant patent ductus arteriosus; IMV, invasive mechanical ventilation; IVH, intraventricular hemorrhage; PRBC, packed red blood cell; ROP, retinopathy of prematurity; SHAP, the Shapley additive explanation.

```{r Evaluation on paper 2 - plot, include=FALSE}
paper_evaluation_plot2 <-
  ggarrange(
    ggarrange(
      ggarrange(
        NULL, paper_dca + theme(legend.position = "none"), NULL
      ...
      , nrow = 2, ncol = 1, heights = c(0.75 + 0.35, 1.75 + 0.65)
      , labels = LETTERS[1:2], font.label = list(size = 7)
    )
    , paper_timing
    , nrow = 1, ncol = 2, widths = c(2.25, 3.26181)
    , labels = c("", LETTERS[3]), font.label = list(size = 7)
  )
```

```{r Evaluation on paper 2 - show, echo=FALSE, fig.height=3.5, fig.width=5.51181}
paper_evaluation_plot2
```

Figure 65. Evaluation of the best prediction models for: (A) clinical utility by decision curves using test set; (B) discrimination by ROC curves using test set; and (C) best timing for each outcome and the required predictors. The color legend of panel B applies for panels A to B. The points in panels A and B respectively indicate residual net benefits and specificities and sensitivities of the best model using the selected threshold. AKI, acute kidney injury; BPD, bronchopulmonary dysplasia; EUGR, extrauterine growth restriction; EUGR-BW, EUGR by body weight; EUGR-HC, EUGR by head circumference; hs-PDA, hemodynamically-significant patent ductus arteriosus; IMV, invasive mechanical ventilation; IVH, intraventricular hemorrhage; PRBC, packed red blood cell; ROP, retinopathy of prematurity.

```{r Severe ROP SHAPbee - show, echo=FALSE, fig.height=2, fig.width=5.51181}
paper_stacked_ori_shap_feature_values$rop_severe_stack_rr |>
  inner_join(
    paper_stacked_ori_shap_feature_values$rop_severe_stack_rr |>
      filter(feature != "Expected value") |>
      group_by(feature) |>
      summarize(max_shap_value = max(shap_value)) |>
      arrange(desc(max_shap_value)) |>
      slice(seq(14)) |>
      select(-max_shap_value)
    , by = join_by(feature)
  ) |>
  mutate(model = "Severe ROP") |>
  paper_shap_beeswarm_plot(
    1, Inf, seed, 0.75, "Expected value"
    , point_size = 0.75, colorbar_height = 6
  ) +
  theme(strip.text = element_blank())
```

Figure 66. Severe ROP prediction model explainability by SHAP beeswarm plot. Only predictors with significant positive impact on the risk are shown. See the impacts of all predictors in Figure 57. AKI, acute kidney injury; IMV, invasive mechanical ventilation; IVH; intraventricular hemorrhage; PRBC, packed red blood cell; ROP, retinopathy of prematurity; SHAP, the Shapley additive explanation; SIP, spontaneous intestinal perforation.

```{r Mod/sev BPD SHAPbee - show, echo=FALSE, fig.height=1.5, fig.width=5.51181}
paper_stacked_ori_shap_feature_values$bpd_moderate_severe_stack_rf |>
  inner_join(
    paper_stacked_ori_shap_feature_values$bpd_moderate_severe_stack_rf |>
      filter(feature != "Expected value") |>
  ...
  paper_shap_beeswarm_plot(
    1, Inf, seed, 0.75, "Expected value"
    , point_size = 0.75, colorbar_height = 5
  ) +
  theme(strip.text = element_blank())
```

Figure 67. Moderate/severe BPD prediction model explainability by SHAP beeswarm plot. Only predictors with significant positive impact on the risk are shown. See the impacts of all predictors in Figure 58. IMV, invasive mechanical ventilation; SHAP, the Shapley additive explanation.

```{r EUGR-HC SHAPbee - show, echo=FALSE, fig.height=2.5, fig.width=5.51181}
paper_stacked_ori_shap_feature_values$eugr_hc_stack_rr |>
  inner_join(
    paper_stacked_ori_shap_feature_values$eugr_hc_stack_rr |>
      filter(feature != "Expected value") |>
...
Figure 68. EUGR-HC prediction model explainability by SHAP beeswarm plot. Only predictors with significant positive impact on the risk are shown. See the impacts of all predictors in Figure 59. AKI, acute kidney injury; EUGR-HC, extrauterine growth restriction by head circumference; IMV, invasive mechanical ventilation; IVH; intraventricular hemorrhage; SHAP, the Shapley additive explanation; SIP, spontaneous intestinal perforation.

```{r EUGR-BW SHAPbee - show, echo=FALSE, fig.height=2, fig.width=5.51181}
paper_stacked_ori_shap_feature_values$eugr_bw_stack_rr |>
  inner_join(
    paper_stacked_ori_shap_feature_values$eugr_bw_stack_rr |>
      filter(feature != "Expected value") |>
...
Figure 69. EUGR-BW prediction model explainability by SHAP beeswarm plot. Only predictors with significant positive impact on the risk are shown. See the impacts of all predictors in Figure 60. AKI, acute kidney injury; EUGR-BW, extrauterine growth restriction by body weight; IMV, invasive mechanical ventilation; SHAP, the Shapley additive explanation.

```{r Failed hearing SHAPb - show, echo=FALSE, fig.height=1.5, fig.width=5.51181}
paper_stacked_ori_shap_feature_values$hearing_stack_rr |>
  inner_join(
    paper_stacked_ori_shap_feature_values$hearing_stack_rr |>
      filter(feature != "Expected value") |>
  ...
  paper_shap_beeswarm_plot(
    1, Inf, seed, 0.75, "Expected value"
    , point_size = 0.75, colorbar_height = 4
  ) +
  theme(strip.text = element_blank())
```

Figure 70. Failed-to-pass-hearing-test prediction model explainability by SHAP beeswarm plot. Only predictors with significant positive impact on the risk are shown. See the impacts of all predictors in Figure 61. SHAP, the Shapley additive explanation.

```{r Identify loss-to-follow-up distribution, include=FALSE}
loss_to_follow_up_dist <-
  cleaned_data |>
  select(id, ga_wk, death_age) |>
  unique() |>
  mutate(
    set =
      case_when(
  ...
  mutate(
    death_age = 
      case_when(
        death_age <= 3 ~ "≤3"
        , death_age > 3 & death_age <= 7 ~ ">3 to 7"
        , death_age > 7 & death_age <= 14 ~ ">7 to 14"
        , death_age > 14 & death_age <= 28 ~ ">14 to 28"
        , death_age > 28 & death_age <= 42 ~ ">28 to 42"
        , death_age > 42 ~ ">42"
      )
    , death_age =
      death_age |>
      factor(c("≤3", ">3 to 7", ">7 to 14", ">14 to 28", ">28 to 42", ">42"))
  ) |>
  arrange(death_age) |>
  group_by(set) |>
  mutate(total = n()) |>
  ungroup() |>
  group_by(set, total, death_age) |>
  summarize(n = n(), .groups = "drop")
```

```{r Loss-to-follow-up samples per day category - create, include=FALSE}
table25 <-
  loss_to_follow_up_dist |>
  select(death_age, set, n) |>
  spread(set, n, fill = 0) |>
  mutate(Modeling = Training + Validation + Test) |>
  gather(set, n, -death_age) |>
  mutate_at("set", \(x) factor(x, unique(x))) |>
  group_by(set) |>
  mutate(n = cumsum(n)) |>
  ungroup() |>
  spread(set, n, fill = 0) |>
  rename(`Death age category` =  death_age)
```

```{r Loss-to-follow-up samples per day category - show, echo=FALSE}
table25 |>
  kable(
    caption = "Table 25. Cumulative loss-to-follow-up samples per day category."
    , format = kable_format
  ) |>
  kable_classic() |>
  column_spec(seq(ncol(table25)), extra_css = "vertical-align:top;")
```

```{r Remaining samples per day dategory- create, include=FALSE}
table26 <-
  loss_to_follow_up_dist |>
  select(death_age, set, n) |>
  spread(set, n, fill = 0) |>
  ...
  mutate_at("set", \(x) factor(x, unique(x))) |>
  group_by(set) |>
  mutate(remaining = total - cumsum(n)) |>
  ungroup() |>
  select(-n, -total) |>
  spread(set, remaining, fill = 0) |>
  rename(`Death age category` =  death_age)
```

```{r Remaining samples per day dategory- show, echo=FALSE}
table26 |>
  kable(
    caption = "Table 26. Remaining samples per day category."
    , format = kable_format
  ) |>
  kable_classic() |>
  column_spec(seq(ncol(table26)), extra_css = "vertical-align:top;")
```

```{r Summary of predictive performance on paper - create, include=FALSE}
table27 <-
  cm_selected_time_summary |>
  filter(!outcome %in% c("survive", "bpd")) |>
  mutate(dataset = "Internal") |>
  ...
  mutate(
    avg =
      ifelse(
        metric == "F1", format(avg, digits = 2), round(avg)
      )
    , lb =
      ifelse(
        metric == "F1", format(lb, digits = 2), round(lb)
      )
    , ub =
      ifelse(
        metric == "F1", format(ub, digits = 2), round(ub)
      )
  ) |>
  unite(ci, lb, ub, sep = ", ") |>
  mutate_at("ci", \(x) paste0("(", x, ")")) |>
  ...
  mutate(
    ...
    , timing = as.character(timing)
    , timing = factor(timing, unique(timing))
    , metric = factor(metric, c("AUC-ROC", "TPR", "PPV", "F1", "TNR", "NPV"))
  ) |>
  spread(metric, summary) |>
  left_join(
    reduce(imap(th, ~ data.frame(model = .y, TH = .x)), rbind) |>
  ...
  select(-timing, -TH) |>
  rename(Outcome = outcome, Dataset = dataset)
```

```{r Summary of predictive performance on paper - show, echo=FALSE}
table27 |>
  kable(
    caption =
      "Table 27. Predictive performance using test set at particular time."
    , format = kable_format
  ) |>
  footnote(
    paste0(
